...
def sanear_base_datos():
    """Limpieza de registros duplicados huérfanos (v10.8.54)"""
    try:
        conn = conectar_db()
...
def generar_pdf_blob(df, titulo, incluir_precios=True):
    try:
        ...
        # Filtro de columnas según necesidad
        ...
        elif "Ajustes" in df_pdf.columns: # Requerimiento: PDF Costos sin columna Ajuste
...
def modulo_dashboard():
    ...
    with c1: st.metric("DEUDA TOTAL", f"${f_puntos(df_f_reales['monto_total'].sum())}")
    with c5: st.metric("PETRÓLEO", f"{f_decimal(saldo_pet)}L")
...
def modulo_petroleo():
    ...
    with tp2:
        with st.form("p_s"):
            ...
            if st.form_submit_button("REGISTRAR SALIDA"):
                # Calculo de PMP para imputación
    ...
    with tp3:
        ...
        # BOTÓN PDF PETRÓLEO
        st.download_button("📥 Descargar Historial PDF", generar_pdf_blob(df_p, "HISTORIAL PETROLEO"), "petroleo.pdf")
        id_p = st.selectbox("ID borrar", df_p['id']); cl = st.text_input("Master", type="password", key="cl_pet")
...
def modulo_compras():
    ...
    with t1:
        ...
        if st.session_state.get('car'):
            ...
            if st.button("💾 GUARDAR FACTURA"):
                ...
                conn.commit(); registrar_accion("FACTURA", nro); st.session_state['car'] = []; guardar_en_drive(); st.rerun()
    with t2:
    ...
    with t3:
        ...
        # BOTÓN PDF HISTORIAL COMPRAS
        st.download_button("📥 Descargar Historial PDF", generar_pdf_blob(df_h, f"HISTORIAL COMPRAS {d_f1} AL {d_f2}"), "compras.pdf")
        id_e = st.selectbox("ID factura", df_h['id']); cl = st.text_input("Master", type="password", key="cl_h_c")
        if st.button("🗑️ ELIMINAR") and cl == CLAVE_MAESTRA:
...
def modulo_tesoreria():
    ...
    with tp1:
        ...
        st.download_button("📥 Descargar Listado Pendientes PDF", generar_pdf_blob(df_p.drop(columns=['id']), "LISTADO PENDIENTES DE PAGO"), "pendientes.pdf")
    ...
    with tp2:
        ...
        if not df_provs.empty:
            ...
            st.download_button(f"📥 Descargar Deuda {p_sel} PDF", generar_pdf_blob(df_det, f"DEUDA PENDIENTE: {p_sel}"), f"deuda_{p_sel}.pdf")
    conn.close()
...
def modulo_bodega():
    ...
    with tb1:
        ...
        with c_p1: st.download_button("📥 PDF Admin (PMP)", generar_pdf_blob(df_s.drop(columns=['id']), "INVENTARIO ADMIN", True), "stock_admin.pdf")
        with c_p2: st.download_button("📥 PDF Campo", generar_pdf_blob(df_s.drop(columns=['id']), "LISTADO STOCK", False), "stock_trabajador.pdf")
        ...
        if st.button("✏️ MODIFICAR") and st.text_input("Master", type="password", key="cl_b_mod") == CLAVE_MAESTRA:
            conn.execute("UPDATE inventario SET producto=?, stock=? WHERE id=?", (n_nom, n_st, id_g)); conn.commit(); st.rerun()
    with tb2:
    ...
    with tb3:
        with st.form("n"):
            ...
            if st.form_submit_button("➕ CREAR"):
                conn.execute("INSERT INTO inventario (producto, familia, stock, precio_medio) VALUES (?,?,?,?)", (nom, fam, si, pi)); conn.commit(); st.rerun()
    with tb4:
        ...
        st.dataframe(df_cc.style.format({"valor_imputado": "${:,.0f}"}), use_container_width=True)
    conn.close()
...
def modulo_costos():
    ...
    with tabs[0]:
        ...
        if not df_r.empty:
            ...
            # BOTÓN PDF COSTOS (Sin columna Ajuste por diseño en función generar_pdf_blob)
            st.download_button("📥 Descargar Informe PDF (Sin Ajustes)", generar_pdf_blob(df_r, "INFORME DE COSTOS POR CUARTEL"), "costos.pdf")
    if es_admin:
        with tabs[1]:
...
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.54", layout="wide")
inicializar_db()
