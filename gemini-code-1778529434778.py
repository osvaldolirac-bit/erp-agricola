...
def modulo_compras():
    ...
    with t3:
        ...
        if not df_h.empty:
            ...
            st.divider(); id_sel = st.selectbox("ID", df_h['id']); cl = st.text_input("Clave", type="password")
...
def modulo_tesoreria():
    st.header("💸 Tesorería")
    tp1, tp2 = st.tabs(["🔴 Pendientes", "🏢 Proveedor"])
    conn = conectar_db()
    with tp1:
        ...
        if not df_p.empty:
            ...
            id_p = st.selectbox("ID Pago", df_p['id']); met = st.selectbox("Medio", ["Transferencia", "Efectivo"])
            if st.button("💰 PAGAR"):
...
def modulo_bodega():
    ...
    with tb1:
        ...
        if not df_s.empty:
            id_ins = st.selectbox("Insumo", df_s['id']); cl = st.text_input("Clave Maestro", type="password", key="cbod")
            if st.button("🗑️ ELIMINAR") and cl == CLAVE_MAESTRA:
    ...
    with tb2:
        ...
        with st.form("mov_form"):
            ...
            cc = st.selectbox("CC", CENTROS_COSTO) if tipo == "Salida (Campo)" else ""
    ...
    with tb3:
        with st.form("nuevo_p"):
            ...
            if st.form_submit_button("CREAR"):
                conn.execute("INSERT INTO inventario (producto, familia, stock) VALUES (?,?,0)", (n, f)); conn.commit(); guardar_en_drive(); st.rerun()
    with tb4:
...
def modulo_costos():
    ...
    if not df_t.empty:
        ...
        st.download_button("📥 PDF Costos", generar_pdf_blob(df_t, "COSTOS"), "costos.pdf")
