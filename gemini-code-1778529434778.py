...
if 'db_sincronizada' not in st.session_state:
    if descargar_de_drive():
        st.toast("✅ Datos recuperados de Google Drive", icon="☁️")
...
def generar_pdf(df, titulo):
    try:
        ...
        pdf.cell(0, 10, titulo, ln=True, align="C"); pdf.ln(5); pdf.set_font("Arial", "B", 8)
...
# --- 2. COMPRAS ---
    ...
    with t1:
        ...
        if st.session_state['carrito']:
            ...
            st.info(f"Neto: ${f_puntos(neto)} | IVA: ${f_puntos(neto*0.19)} | Total: ${f_puntos(total_sugerido)}")
...
# --- 4. BODEGA ---
    ...
    tb1, tb2, tb3 = st.tabs(["📊 Stock", "🔄 Movimientos", "➕ Nuevos Insumos"])
    ...
    with tb3:
        with st.form("new"):
            ...
            n_p = st.text_input("Nombre"); f_p = st.selectbox("Familia", ["Fertilizante", "Petróleo", "Herbicida", "Otros"])
