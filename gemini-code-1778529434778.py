...
def inyectar_css():
    st.markdown("""<style>
        ...
        .module-card { padding: 20px; border-radius: 15px; text-align: center; color: white; margin-bottom: 10px; }
...
def modulo_dashboard():
    ...
    # 2. Resumen Visual por Módulos
    c1, c2, c3, c4, c5 = st.columns(5)
    c1.markdown("<div class='module-card' style='background-color:#455a64;'>⛽<br>Petróleo</div>", unsafe_allow_html=True)
    c2.markdown("<div class='module-card' style='background-color:#1976d2;'>📦<br>Compras</div>", unsafe_allow_html=True)
    c3.markdown("<div class='module-card' style='background-color:#2E7D32;'>💸<br>Tesorería</div>", unsafe_allow_html=True)
    c4.markdown("<div class='module-card' style='background-color:#f57c00;'>🏠<br>Bodega</div>", unsafe_allow_html=True)
    c5.markdown("<div class='module-card' style='background-color:#ffd700; color:black;'>💰<br>Costos</div>", unsafe_allow_html=True)
...
def modulo_petroleo():
    ...
    with t3:
        ...
        id_del = st.selectbox("ID a borrar", df_p['id']); cl = st.text_input("Clave Master", type="password", key="cl_p")
...
def modulo_bodega():
    ...
    with t3:
        with st.form("nuevo_insumo"):
            n_p = st.text_input("Producto"); n_f = st.selectbox("Familia", FAMILIAS_PRODUCTOS); n_s = st.number_input("Stock", 0.0); n_pn = st.number_input("Precio Neto", 0.0)
...
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.63", layout="wide")
inicializar_db()
