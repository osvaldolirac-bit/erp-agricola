...
# --- DATA DE INYECCIÓN EL ESPINO (GLOBAL) ---
DATA_ESP_HISTORICA = [
...
def guardar_en_drive():
    ...
    if drive:
        ...
        st.success("✅ Respaldo sincronizado.")
    ...
    try:
        ...
        for _, row in df_p.iterrows():
            for i, item in enumerate(row):
                ...
                elif any(x in col_n for x in ["litros", "cantidad", "stock"]): val = f_decimal(item)
                else: val = str(item)
...
def modulo_compras():
    ...
    with t2:
        pg, ng, fg1, fg2 = st.text_input("Prov Gasto", key="pg"), st.text_input("N° Gasto", key="ng"), st.date_input("F. Compra", hoy), st.date_input("F. Vence", hoy)
    ...
    with t3:
        ...
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            id_del = st.selectbox("ID factura a borrar", df_h['id']); clv_c = st.text_input("Clave Compras", type="password", key="cl_com")
...
def modulo_espino():
    ...
    t1, t2 = st.tabs(["➕ Registro", "📜 Historial de Gastos"])
    conn = conectar_db()
    with t1:
        ...
        with st.form("esp_f"):
            ...
            if st.form_submit_button("💾 GUARDAR GASTO"):
                ...
                conn.commit(); registrar_accion("GASTO ESPINO", d); st.rerun()
    with t2:
...
def modulo_tesoreria():
    ...
    with t1:
        ...
        id_p = st.selectbox("ID a Pagar", df_p['id']); met = st.selectbox("Método de Pago", ["Transferencia", "Efectivo", "Cheque"])
    ...
    with t2:
        ...
        if not df_prv.empty:
            ...
            st.download_button(f"📥 PDF {prv}", generar_pdf_blob(df_pr, f"ESTADO CUENTA: {prv}"), f"deuda_{prv}.pdf")
    conn.close()
...
def modulo_bodega():
    ...
    with t1:
        ...
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            st.divider(); id_b = st.selectbox("ID Insumo a Editar", df_s['id']); item = df_s[df_s['id']==id_b].iloc[0]
            n_nom = st.text_input("Nuevo Nombre", item['producto']); n_st = st.number_input("Corregir Stock", value=float(item['stock']))
    ...
    with t4:
        cc_q = st.selectbox("Cuartel de Consulta", CENTROS_COSTO)
...
st.set_page_config(page_title="ERP v10.8.90", layout="wide")
inicializar_db()
