...
# --- 2. FUNCIONES DE UTILIDAD Y MOTOR DE BASE DE DATOS ---
...
def sanear_y_recalcular():
    try:
        conn = conectar_db()
        # Limpieza de prorrateos huérfanos
...
def inicializar_db():
    ...
    # --- 💾 INYECCIÓN MASIVA DE DATOS EL ESPINO (Regla No Resumir) ---
    ...
    if cursor.fetchone()[0] == 0:
        datos_ex = [
            ...
            ('2026-03-03', '2237580', 'Coagra Urea granulada', 198417), ('2026-03-06', '6966966', 'Electrocom Contractor/Cable', 220326),
            ...
            ('2026-03-30', 'S/N', 'Carlos Zavala Sueldo Marzo', 620000), ('2026-03-18', 'S/N', 'CGE Consumo Eléctrico', 309600),
            ...
            ('2026-03-11', 'S/N', 'Gustavo Carreño Contador', 315000), ('2026-03-11', 'S/N', 'Carlos Lira V.', 243882),
            ('2026-03-11', '349905', 'Equipos Riego Motor sumergible', 167171), ('2026-03-11', '1427603', 'Vitel Cable reviflex', 108469),
            ('2026-03-11', '6954495', 'Electrocom Tubo curvable', 27703), ('2026-03-11', 'S/N', 'Imposiciones CZ feb', 143483),
            ('2026-04-02', 'S/N', 'Alejandra Leviman Bono', 12500), ('2026-04-02', 'S/N', 'Alejandra Leviman sueldo', 112500),
            ...
            ('2026-04-30', 'S/N', 'Cáceres M SPA Control heladas', 4545184), ('2026-05-08', 'BCI', 'Comisión tarjeta', 13368),
            ('2026-05-12', 'S/N', 'CZ Imposiciones Abril', 143914), ('2026-05-15', '19509', 'Sendai Datalogger Rc-17', 58362),
            ('2026-05-17', 'Arriendo', 'María Paola Torrez Ortiz', 7000000)
        ]
        cursor.executemany("INSERT INTO gastos_espino (fecha, documento, item, monto) VALUES (?,?,?,?)", d_ex)
    conn.commit()
    conn.close()
    sanear_y_recalcular()
...
def generar_pdf_blob(df, titulo, incluir_precios=True, total_manual=None, modo_petroleo=False):
    try:
        ...
        w = 190 / len(cols)
        for col in cols:
            pdf.cell(w, 8, str(col).upper(), border=1, align="C")
        pdf.ln()
        pdf.set_font("Helvetica", "", 7)
        for _, row in df_p.iterrows():
            for i, item in enumerate(row):
                col_n = df_p.columns[i].lower()
                if any(x in col_n for x in ["monto", "total", "valor"]):
                    val = f_puntos(item)
                elif any(x in col_n for x in ["litros", "cantidad", "stock"]):
                    val = f_decimal(item)
                else:
                    val = str(item)
        ...
        if incluir_precios and total_manual is not None:
            pdf.set_font("Helvetica", "B", 9)
            pdf.cell(w * (len(cols)-1), 8, "TOTAL FINAL:", border=1, align="R")
            pdf.cell(w, 8, f"${f_puntos(total_manual)}", border=1, align="L")
...
# --- 4. MÓDULOS DEL SISTEMA (ORDENADOS Y COMPLETOS) ---
...
def modulo_dashboard():
    ...
    # Cálculo de Petróleo para Métrica
    ...
    with m1:
        st.markdown(f"<div class='stMetric'><small>💰 DEUDA TOTAL</small><br><span class='metric-big'>${f_puntos(df_f['monto_total'].sum())}</span></div>", unsafe_allow_html=True)
    
    p_dia = hoy.replace(day=1)
    d_critica = df_f[pd.to_datetime(df_f['fecha_vencimiento']).dt.date < p_dia]['monto_total'].sum()
    with m2:
        st.markdown(f"<div class='stMetric card-critico'><small>🔥 MESES ANTERIORES</small><br><span class='metric-red'>${f_puntos(d_critica)}</span><br><small style='color:red;'>CRÍTICO</small></div>", unsafe_allow_html=True)
    
    v_count = len(df_f[pd.to_datetime(df_f['fecha_vencimiento']).dt.date < hoy])
    with m3:
        st.markdown(f"<div class='stMetric card-vencida'><small>⚠️ VENCIDAS</small><br><span class='metric-blue'>{v_count}</span><br><small style='color:#1976d2;'>DOCUMENTOS</small></div>", unsafe_allow_html=True)
    
    with m4:
        st.markdown(f"<div class='stMetric'><small>📄 PENDIENTES</small><br><span style='font-size:2rem; font-weight:600;'>{len(df_f)}</span><br><small>CARTERA</small></div>", unsafe_allow_html=True)
    
    with m5:
        st.markdown(f"<div class='stMetric'><small>⛽ PETRÓLEO</small><br><span style='font-size:2rem; font-weight:600;'>{f_decimal(saldo_pet)}L</span><br><small>NETO</small></div>", unsafe_allow_html=True)
    
    st.divider()
    c_izq, c_der = st.columns([1.6, 1])
    with c_izq:
        st.markdown("### 📊 Resumen Gastos Netos por Cuartel")
    ...
    with c_der:
        st.markdown("### 📅 Proyección de Pagos 4 Meses")
        for i in range(4):
...
def modulo_petroleo():
    ...
    t1, t2, t3 = st.tabs(["📥 Carga", "🚜 Salida", "📊 Historial"])
    conn = conectar_db()
    ...
    with t2:
        with st.form("p_salida"):
            ...
            if st.form_submit_button("REGISTRAR SALIDA"):
                ...
                for c in ccs:
                    conn.execute("INSERT INTO petroleo (tipo, litros, vehiculo, responsable, centro_costo, fecha, valor_imputado) VALUES (?,?,?,?,?,?,?)", (ls/len(ccs), v, r, c.upper(), hoy, (ls/len(ccs)*pmp)))
                conn.commit()
                st.rerun()
    ...
    with t3:
        ...
        saldo_hist = (df_l_c['l'].fillna(0).iloc[0]) - (df_l_s['l'].fillna(0).iloc[0])
        st.info(f"### SALDO ACTUAL EN TANQUE: {f_decimal(saldo_hist)} LITROS")
        ...
        # Seguridad Modificar/Eliminar
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            st.divider()
            id_p = st.selectbox("ID Petróleo a corregir", df_p['id'])
            clv = st.text_input("Clave Maestra", type="password", key="cl_pet")
            c1, c2 = st.columns(2)
            if c1.button("🗑️ ELIMINAR REGISTRO") and clv == CLAVE_MAESTRA:
...
def modulo_compras():
    ...
    t1, t2, t3 = st.tabs(["➕ Insumos", "💸 Gastos Varios", "🔍 Historial Completo"])
    conn = conectar_db()
    
    with t1:
        c1, c2 = st.columns(2)
        nro, prov, fe, fv = c1.text_input("N° Documento"), c1.text_input("Proveedor"), c2.date_input("Fecha Emisión"), c2.date_input("Vencimiento")
    ...
    with t2:
        pg, ng, fg1, fg2 = st.text_input("Proveedor Gasto", key="pg"), st.text_input("N° Doc Gasto", key="ng"), st.date_input("Fecha Gasto", hoy), st.date_input("Vence Gasto", hoy)
        sel_cc = [cc for cc in CENTROS_COSTO if st.checkbox(cc, key=f"gv_{cc}")]
        mt = st.number_input("Total Bruto ($)", 0.0)
        iva = st.radio("¿Imputar valor TOTAL (incluyendo IVA)?", ["SÍ (BRUTO)", "NO (NETO)"])
        if st.button("💾 GUARDAR GASTO VARIO"):
            imp = mt if iva == "SÍ (BRUTO)" else mt/1.19
    ...
    with t3:
        ...
        f_ini_c = colf1.date_input("Desde", hoy - timedelta(days=365))
        f_fin_c = colf2.date_input("Hasta", hoy)
        df_h = pd.read_sql_query(f"SELECT id, nro_documento, proveedor, fecha_compra, monto_total FROM facturas WHERE monto_total > 0 AND nro_documento NOT LIKE '%_P' AND fecha_compra BETWEEN '{f_ini_c}' AND '{f_fin_c}' ORDER BY fecha_compra DESC", conn)
        st.dataframe(df_h.style.format({"monto_total": "${:,.0f}"}), use_container_width=True)
        st.download_button("📥 PDF Historial Compras", generar_pdf_blob(df_h, "HISTORIAL DE COMPRAS"), "compras.pdf")
        
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            id_del_c = st.selectbox("ID Factura a corregir", df_h['id'])
            clv_c = st.text_input("Clave Maestra", type="password", key="cl_compras")
            if st.button("🗑️ ELIMINAR FACTURA") and clv_c == CLAVE_MAESTRA:
                f_sel = df_h[df_h['id']==id_del_c].iloc[0]
                conn.execute("DELETE FROM facturas WHERE id=?", (id_del_c,))
...
def modulo_tesoreria():
    ...
    t1, t2 = st.tabs(["🔴 Pendientes", "🏢 Consulta por Proveedor"])
    conn = conectar_db()
    ...
    with t1:
        ...
        if st.button("💰 PAGAR SELECCIONADO"):
            conn.execute("UPDATE facturas SET estado='Pagado', metodo_pago=?, fecha_pago=? WHERE id=?", (met, hoy, id_pay))
            conn.commit(); st.rerun()
            
    with t2:
        df_provs = pd.read_sql_query("SELECT DISTINCT proveedor FROM facturas WHERE monto_total > 0", conn)
        if not df_provs.empty:
            pr_sel = st.selectbox("Seleccione Proveedor", df_provs['proveedor'])
            df_pr = pd.read_sql_query(f"SELECT nro_documento, fecha_compra, fecha_vencimiento, monto_total, estado FROM facturas WHERE proveedor='{pr_sel}' AND nro_documento NOT LIKE '%_P'", conn)
            st.info(f"### DEUDA PENDIENTE CON {pr_sel}: ${f_puntos(df_pr[df_pr['estado']=='Pendiente']['monto_total'].sum())}")
            st.dataframe(df_pr.style.format({"monto_total": "${:,.0f}"}), use_container_width=True)
            st.download_button(f"📥 PDF Deuda {pr_sel}", generar_pdf_blob(df_pr, f"ESTADO DE CUENTA: {pr_sel}"), f"deuda_{pr_sel}.pdf")
    conn.close()
...
def modulo_bodega():
    ...
    with t1:
        ...
        st.download_button("📥 PDF Stock Admin", generar_pdf_blob(df_s.drop(columns=['id']), "STOCK ADMINISTRATIVO", True), "admin_stock.pdf")
        st.download_button("📥 PDF Stock Campo", generar_pdf_blob(df_s.drop(columns=['id']), "STOCK CAMPO", False), "campo_stock.pdf")
        
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            st.divider()
            id_b = st.selectbox("ID Insumo a Editar", df_s['id'])
            item = df_s[df_s['id']==id_b].iloc[0]
            n_nom = st.text_input("Nuevo Nombre", item['producto'])
            n_st = st.number_input("Corregir Stock", value=float(item['stock']))
            clv = st.text_input("Clave Maestra", type="password", key="cl_bod")
            c1, c2 = st.columns(2)
            if c1.button("✏️ ACTUALIZAR") and clv == CLAVE_MAESTRA:
                conn.execute("UPDATE inventario SET producto=?, stock=? WHERE id=?", (n_nom, round(n_st, 2), id_b))
                conn.commit(); st.rerun()
            if c2.button("🗑️ ELIMINAR") and clv == CLAVE_MAESTRA:
                conn.execute("DELETE FROM inventario WHERE id=?", (id_b,))
                conn.commit(); st.rerun()

    with t2:
        df_i = pd.read_sql_query("SELECT id, producto, precio_medio FROM inventario", conn)
        ps = st.selectbox("Insumo a Salir", df_i['id'].astype(str) + " - " + df_i['producto'])
        ct = st.number_input("Cantidad Salida", 0.0)
        ccs = [cc for cc in CENTROS_COSTO if st.checkbox(cc, key=f"mb_{cc}")]
        if st.button("REGISTRAR SALIDA"):
            iid = int(ps.split(" - ")[0])
            pmp = df_i[df_i['id']==iid]['precio_medio'].iloc[0]
            if ct > 0 and ccs:
                for c in ccs:
                    conn.execute("INSERT INTO movimientos (producto_id, tipo, cantidad, fecha, centro_costo, valor_imputado) VALUES (?,?,?,?,?,?)", (iid, "Salida", ct/len(ccs), hoy, c.upper(), (ct/len(ccs)*pmp)))
                conn.execute("UPDATE inventario SET stock = stock - ? WHERE id = ?", (round(ct, 2), iid))
                conn.commit(); st.rerun()

    with t3:
        with st.form("nuevo_ins"):
            np, nf = st.text_input("Nombre del Insumo"), st.selectbox("Familia", FAMILIAS_PRODUCTOS)
            ns, npr = st.number_input("Stock Inicial", 0.0), st.number_input("PMP Neto Inicial", 0.0)
    ...
    with t4:
        cc_q = st.selectbox("Filtrar por Cuartel", CENTROS_COSTO)
        ...
        st.download_button(f"📥 PDF Consulta {cc_q}", generar_pdf_blob(df_cc, f"MOVIMIENTOS EN {cc_q}"), f"cc_{cc_q}.pdf")
    conn.close()

def modulo_espino():
    st.header("🏡 El Espino - Gestión de Gastos")
    es_admin = (st.session_state.get('email') == 'osvaldolira@laconcepcion.cl')
    t1, t2 = st.tabs(["➕ Registro de Gastos", "📜 Historial a la Fecha"])
    conn = conectar_db()
    
    with t1:
        # Pestaña Limpia: Sin totales según pedido
        with st.form("esp_form"):
            f, d, it, mt = st.date_input("Fecha", hoy), st.text_input("Doc/Proveedor"), st.text_input("Ítem Gasto"), st.number_input("Monto ($)", 0.0)
    ...
    with t2:
        ...
        f_ini = col1.date_input("Desde", f_min)
        ...
        # MÉTRICA DE GASTO A LA FECHA (SUMATORIA RANGO)
        ...
        st.download_button("📥 Descargar PDF Historial El Espino", generar_pdf_blob(df_h.drop(columns=['id']), f"REPORTE GASTOS EL ESPINO: {f_ini} AL {f_fin}"), "espino.pdf")
        
        if es_admin and not df_h.empty:
            st.divider()
            id_e = st.selectbox("ID a corregir", df_h['id'])
            item_e = df_h[df_h['id']==id_e].iloc[0]
            n_it = st.text_input("Modificar Descripción", item_e['item'])
            n_mt = st.number_input("Modificar Monto", value=float(item_e['monto']))
            clv = st.text_input("Clave Maestra", type="password", key="cl_e")
            c1, c2 = st.columns(2)
            if c1.button("✏️ ACTUALIZAR GASTO") and clv == CLAVE_MAESTRA:
                conn.execute("UPDATE gastos_espino SET item=?, monto=? WHERE id=?", (n_it, n_mt, id_e))
                conn.commit(); st.rerun()
            if c2.button("🗑️ ELIMINAR GASTO") and clv == CLAVE_MAESTRA:
                conn.execute("DELETE FROM gastos_espino WHERE id=?", (id_e,))
                conn.commit(); st.rerun()
    conn.close()
...
def modulo_costos():
    ...
    es_admin = (st.session_state.get('email') == 'osvaldolira@laconcepcion.cl')
    t1, t2 = st.tabs(["📊 Resumen", "🔧 Ajustes Manuales"])
    conn = conectar_db()
    ...
    with t1:
        ...
        if not df_r.empty:
            ...
            df_m = df_r.copy()
            if not es_admin:
                df_m = df_m.drop(columns=['Ajustes'])
            st.dataframe(df_m.style.format({c: ("${:,.0f}" if c != 'cc' else str) for c in df_m.columns if c != 'cc'}), use_container_width=True)
            st.download_button("📥 PDF Informe de Costos", generar_pdf_blob(df_r, "INFORME DE COSTOS POR CUARTEL", total_manual=tot), "costos.pdf")
            
    with t2:
        if es_admin:
            with st.form("aj_form"):
                cc_aj, m_aj, mot = st.selectbox("Cuartel a Ajustar", CENTROS_COSTO), st.number_input("Monto ($)", 0.0), st.text_input("Motivo")
...
def modulo_seguridad():
    ...
    t1, t2 = st.tabs(["📜 Bitácora de Acciones", "🔑 Registro de Accesos"])
    conn = conectar_db()
    with t1:
        df_b = pd.read_sql_query("SELECT * FROM bitacora ORDER BY id DESC", conn)
        st.dataframe(df_b, use_container_width=True)
        st.download_button("📥 PDF Bitácora", generar_pdf_blob(df_b, "BITACORA DE OPERACIONES"), "bitacora.pdf")
    with t2:
        df_a = pd.read_sql_query("SELECT * FROM log_accesos ORDER BY id DESC", conn)
        st.dataframe(df_a, use_container_width=True)
        st.download_button("📥 PDF Accesos", generar_pdf_blob(df_a, "LOG DE ACCESOS"), "accesos.pdf")
    conn.close()
...
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.88", layout="wide")
inicializar_db()
...
if not st.session_state['logged_in']:
    ...
    with st.sidebar:
        ...
        st.divider()
        m_opts = { 
            "🏠 DASHBOARD": "DASHBOARD", "⛽ Petróleo": "Petróleo", "📦 Compras": "Compras", 
            "💸 Tesorería": "Tesorería", "🏠 Bodega": "Bodega", "🏡 El Espino": "Espino", 
            "💰 Costos": "Costos" 
        }
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            m_opts["🕵️ Seguridad"] = "Seguridad"
            
        menu = m_opts[st.radio("MENÚ DE NAVEGACIÓN", list(m_opts.keys()))]
        st.divider()
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl' and st.button("🚀 Sincronizar Drive"):
            guardar_en_drive()
