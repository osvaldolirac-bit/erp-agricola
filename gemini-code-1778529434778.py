...
def modulo_compras():
    ...
    with t2:
        ...
        fe_g, fv_g = st.columns(2)[0].date_input("Compra", hoy, key="fg1"), st.columns(2)[1].date_input("Vence", hoy, key="fg2")
...
def modulo_bodega():
    st.header("🚜 Bodega")
    tb1, tb2, tb3, tb4 = st.tabs(["📊 Stock", "🔄 Movimientos", "➕ Insumo", "🔍 Consulta CC"])
    conn = conectar_db()
    with tb1:
