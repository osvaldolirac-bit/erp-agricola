...
def guardar_en_drive():
    ...
    if drive:
        ...
        st.success("✅ Respaldo en la nube actualizado.")
...
def sanear_base_datos():
    """Limpieza de duplicados y recalculo de petróleo"""
    try:
        conn = conectar_db()
        # Eliminar particiones sin factura padre
        ...
        # Recalcular petróleo bruto a neto en registros antiguos
...
def inyectar_css():
    st.markdown("""<style>
        ...
        .stButton>button { border-radius: 8px; background-color: #2E7D32; color: white; width: 100%; font-weight: bold; }
        .stTabs [data-baseweb="tab"] { background-color: #e8f5e9; padding: 10px 20px; border-radius: 5px 5px 0 0; }
...
def modulo_dashboard():
    ...
    with col1:
        st.subheader("💰 Resumen Gastos Netos por Cuartel")
        ...
        if not df_c.empty:
            total_g = df_c['total'].sum()
            df_c = pd.concat([df_c, pd.DataFrame([{'cc': 'TOTAL GENERAL', 'total': total_g}])], ignore_index=True)
            st.dataframe(df_c.style.format({"total": "${:,.0f}"}), use_container_width=True)
    with col2:
        st.subheader("📅 Proyección de Gasto (4 Meses)")
        ...
        for i in range(4):
            ...
            st.markdown(f"<div style='background:white; padding:10px; border-radius:8px; margin-bottom:5px; display:flex; justify-content:space-between; border:1px solid #ddd;'><b>{meses_n[f_p.month-1]} {f_p.year}</b> <span style='color:#2E7D32;'>${f_puntos(total_m)}</span></div>", unsafe_allow_html=True)
    conn.close()
...
def modulo_petroleo():
    ...
    with tp2:
        with st.form("p_s"):
            lts_s, veh, res, ccs_sel = st.number_input("Litros", 0.0), st.text_input("Vehículo"), st.text_input("Responsable"), []
            cols = st.columns(3)
            ...
            if st.form_submit_button("REGISTRAR SALIDA"):
                ...
                for c in ccs_sel:
                    conn.execute("INSERT INTO petroleo (tipo, litros, vehiculo, responsable, centro_costo, fecha, valor_imputado) VALUES (?,?,?,?,?,?,?)", 
                                 ("Salida", lts_s/len(ccs_sel), veh, res, c.upper(), hoy, (lts_s/len(ccs_sel)*pmp_n)))
...
def modulo_compras():
    ...
    with t2:
        ...
        mt = st.number_input("Total Factura (IVA inc.)", 0.0); iva = st.radio("¿Imputar TOTAL al CC?", ["SÍ", "NO (NETO)"])
        if st.button("💾 GUARDAR GASTO"):
            if conn.execute("SELECT COUNT(*) FROM facturas WHERE nro_documento=? AND proveedor=?", (ng, pg)).fetchone()[0] > 0:
                st.error("⚠️ Factura ya existe."); return
    ...
    with t3:
        ...
        id_del = st.selectbox("ID Factura", df_h['id']); cl = st.text_input("Clave Master", type="password", key="cl_h")
        if st.button("🗑️ ELIMINAR") and cl == CLAVE_MAESTRA:
...
def modulo_tesoreria():
    ...
    with tp1:
        ...
        id_pay = st.selectbox("ID Factura", df_p['id']); met = st.selectbox("Método", ["Transferencia", "Efectivo", "Cheque"])
        if st.button("💰 PAGAR"):
...
def modulo_bodega():
    st.header("🚜 Bodega")
    tb1, tb2, tb3, tb4 = st.tabs(["📊 Stock Actual", "🔄 Salida", "➕ Nuevo Registro", "🔍 Consulta CC"]); conn = conectar_db()
    with tb1:
    ...
    with tb3:
        st.subheader("➕ Crear Insumo Nuevo")
        with st.form("nuevo_insumo"):
            nom_n = st.text_input("Nombre del Insumo")
            fam_n = st.selectbox("Familia del Producto", FAMILIAS_PRODUCTOS)
            stk_n = st.number_input("Stock Inicial", 0.0)
            pmp_n = st.number_input("Precio Neto Unitario", 0.0)
            if st.form_submit_button("REGISTRAR EN INVENTARIO"):
                if nom_n:
                    conn.execute("INSERT INTO inventario (producto, familia, stock, precio_medio) VALUES (?,?,?,?)", (nom_n, fam_n, round(stk_n, 2), pmp_n))
                    conn.commit(); st.success("Insumo creado!"); st.rerun()
    with tb4:
...
def modulo_costos():
    ...
    tabs = st.tabs(["📊 Resumen por Cuartel", "🔧 AJUSTE"]) if es_admin else st.tabs(["📊 Resumen por Cuartel"])
    conn = conectar_db()
    with tabs[0]:
        query = """SELECT UPPER(TRIM(cc)) as cc, 
    ...
    if es_admin:
        with tabs[1]:
            cc_aj = st.selectbox("Cuartel", CENTROS_COSTO); m_aj = st.number_input("Monto (+/-)"); mot = st.text_input("Motivo")
...
def modulo_seguridad():
    st.header("🕵️ Auditoría y Seguridad")
    conn = conectar_db()
    t1, t2 = st.tabs(["📜 Bitácora de Movimientos", "🔑 Historial de Accesos"])
    with t1:
    ...
    with col2:
        with st.form("login_pro"):
            ...
            if st.form_submit_button("ACCEDER AL SISTEMA"):
                ...
                if cursor.fetchone():
                    # REPARACIÓN REGISTRO ACCESOS (Commit inmediato)
                    ...
                    conn.close(); st.error("Acceso incorrecto.")

# --- 6. NAVEGACIÓN ---
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.60", layout="wide")
inicializar_db()
...
if not st.session_state['logged_in']:
    ...
    with st.sidebar:
        ...
        st.divider()
        # Sidebar con Mayúsculas en DASHBOARD
