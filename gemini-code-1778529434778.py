...
def modulo_dashboard():
    ...
    # Consulta de Costos Consolidada (Sin Repetidos)
    query_c = """
    SELECT centro_costo, SUM(val) as total_neto FROM (
...
def modulo_compras():
    ...
    with t2:
        with st.form("form_gv"):
            ...
            cc = st.selectbox("Imputar a Cuartel", CENTROS_COSTO); iva = st.radio("¿Cargar IVA al costo?", ["SÍ (Total)", "NO (Neto)"])
    ...
    with t3:
        ...
        if not df_h.empty:
            ...
            cl = st.text_input("Clave de Seguridad", type="password", key="cl_compras")
...
def modulo_tesoreria():
    ...
    with tp1:
        ...
        if not df_p.empty:
            st.dataframe(df_p.style.apply(estilo_vencimiento, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True)
    ...
    with tp2:
        ...
        if not df_pr.empty:
            p_sel = st.selectbox("Proveedor", df_pr['proveedor']); df_f = pd.read_sql_query(f"SELECT nro_documento, fecha_vencimiento, monto_total FROM facturas WHERE estado='Pendiente' AND proveedor='{p_sel}'", conn)
    ...
    with tp3:
        ...
        if not df_r.empty:
            ...
            st.download_button("📥 PDF Rango Selección", generar_pdf_blob(df_r, "VENCIMIENTOS RANGO"), "venc_rango.pdf")
    conn.close()
...
def modulo_bodega():
    ...
    with tb1:
        ...
        if not df_s.empty:
            ...
            cl = st.text_input("Clave de Seguridad (Bodega)", type="password", key="cl_bod")
            col1, col2 = st.columns(2)
            if col1.button("ACTUALIZAR BODEGA") and cl == CLAVE_SEGURIDAD:
    ...
    with tb2:
        ...
        with st.form("form_mov"):
            ...
            ps = st.selectbox("Producto", df_i['id'].astype(str) + " - " + df_i['producto']); ct, cc = st.number_input("Cant", min_value=0.01), st.selectbox("Centro Costo", CENTROS_COSTO)
...
def modulo_costos():
    ...
    if not df_t.empty: st.download_button("📥 PDF Reporte de Costos", generar_pdf_blob(df_t, "REPORTE CONSOLIDADO DE COSTOS"), "costos.pdf")

# --- 5. NAVEGACIÓN ---
st.set_page_config(page_title="LA CONCEPCIÓN ERP v8.2", layout="wide")
