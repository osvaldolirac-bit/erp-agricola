...
def sanear_y_netear_datos():
    """Limpieza de duplicados y ajuste a NETO"""
    try:
        conn = conectar_db()
...
def modulo_compras():
    ...
    with t3:
        ...
        id_del = st.selectbox("ID a borrar", df_h['id']); cl = st.text_input("Master", type="password", key="cl_h")
        if st.button("🗑️ ELIMINAR") and cl == CLAVE_MAESTRA:
...
def modulo_bodega():
    ...
    with tb3:
        # REPARACIÓN: Formulario de ingreso nuevo
        with st.form("nuevo_insumo_form"):
            nom_n = st.text_input("Nombre del Producto")
            fam_n = st.selectbox("Familia", FAMILIAS_PRODUCTOS)
            stock_n = st.number_input("Stock Inicial", value=0.0)
            pmp_n = st.number_input("Precio Neto Inicial", value=0.0)
            if st.form_submit_button("➕ CREAR PRODUCTO"):
                if nom_n:
                    conn.execute("INSERT INTO inventario (producto, familia, stock, precio_medio) VALUES (?,?,?,?)", (nom_n, fam_n, round(stock_n, 2), pmp_n))
                    conn.commit(); st.success("Producto creado!"); st.rerun()
    with tb4:
...
def modulo_costos():
    ...
    if es_admin:
        with tabs[1]:
            ...
            if st.button("💾 APLICAR AJUSTE") and st.text_input("Clave", type="password", key="cl_aj") == CLAVE_MAESTRA:
...
# --- 7. NAVEGACIÓN ---
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.59", layout="wide")
inicializar_db()
