...
CLAVE_MAESTRA = "2908" # Clave para borrar/modificar
...
def inyectar_css():
    st.markdown("""
        ...
        .stButton>button:hover { background-color: #1B5E20; color: white; border: 1px solid white; }
        </style>
...
def modulo_dashboard():
    inyectar_css()
    st.title("🚜 Panel de Administración Agrícola")
    conn = conectar_db()
    ...
    with col_izq:
        ...
        if not df_c.empty:
            ...
            def bold_total(row):
                color = COLORES_CUARTELES.get(row['cc'], "#333")
                if row['cc'] == "TOTAL GENERAL": return ['font-weight: bold; background-color: #E8F5E9; color: black' for _ in row]
                return [f'border-left: 5px solid {color}' if i==0 else '' for i in range(len(row))]
...
def modulo_compras():
    ...
    with t3:
        conn = conectar_db(); df_h = pd.read_sql_query("SELECT id, nro_documento, proveedor, fecha_compra, monto_total, estado, tipo, concepto FROM facturas WHERE monto_total > 0 ORDER BY fecha_compra DESC", conn)
        ...
        if not df_h.empty:
            ...
            cl = st.text_input("Clave Maestra para Cambios", type="password", key="cl_h")
...
def modulo_tesoreria():
    st.header("💸 Tesorería y Cuentas por Pagar")
...
def modulo_bodega():
    ...
    with tb1:
        ...
        if not df_s.empty:
            id_ins = st.selectbox("ID Insumo", df_s['id']); n_n, n_p = st.text_input("Nuevo Nombre", df_s[df_s['id']==id_ins]['producto'].values[0]), st.number_input("Nuevo PMP", value=float(df_s[df_s['id']==id_ins]['precio_medio'].values[0]))
    ...
    with tb2:
        ...
        with st.form("mov_form"):
            ...
            if st.form_submit_button("REGISTRAR"):
                ...
                conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
    with tb4:
        cc_sel = st.selectbox("Cuartel Destino", CENTROS_COSTO)
        h1, h2 = st.date_input("Desde  ", hoy-timedelta(days=365)), st.date_input("Hasta  ", hoy)
        df_cc = pd.read_sql_query(f"SELECT m.fecha, i.producto, m.tipo, m.cantidad, m.valor_imputado FROM movimientos m JOIN inventario i ON m.producto_id = i.id WHERE UPPER(TRIM(m.centro_costo)) = '{cc_sel.upper()}' AND m.fecha BETWEEN '{h1}' AND '{h2}' ORDER BY m.fecha DESC", conn); conn.close()
...
def modulo_costos():
    ...
    if not df_t.empty:
        ...
        def bold_total(row): return ['font-weight: bold; background-color: #f1f8e9' if row['cc'] == "TOTAL GENERAL" else '' for _ in row]
...
# --- 8. NAVEGACIÓN ---
st.set_page_config(page_title="ERP v10.0 LA CONCEPCIÓN", layout="wide")
inicializar_db()
...
if not st.session_state['logged_in']: login()
    ...
    with st.sidebar:
        st.title(f"👤 {st.session_state['email']}")
        menu = st.radio("MENÚ", ["🏠 Dashboard", "📦 Compras", "💸 Tesorería", "🚜 Bodega", "💰 COSTOS"])
