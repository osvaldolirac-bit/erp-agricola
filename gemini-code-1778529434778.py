...
DATA_ESP_HISTORICA = [
    ...
    ('2026-03-11', 'S/N', 'Carlos Lira V.', 243882), ('2026-03-11', '349905', 'Equipos Riego Motor 4"', 167171),
...
def modulo_compras():
    ...
    with t1:
        ...
        ct, pr = st.number_input("Cantidad", 0.0), st.number_input("Precio Neto", 0.0)
    ...
    with t2:
        ...
        mt = st.number_input("Total Bruto", 0.0); iva = st.radio("Imputar Bruto?", ["SÍ", "NO (NETO)"])
        if st.button("💾 GUARDAR GASTO VARIO"):
            ...
            conn.commit(); guardar_en_drive(); st.rerun()
    with t3:
        cf1, cf2 = st.columns(2); fi_c = cf1.date_input("Desde", hoy-timedelta(days=365)); ff_c = cf2.date_input("Hasta", hoy)
        ...
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            id_del = st.selectbox("ID eliminar", df_h['id']); clv_c = st.text_input("Clave", type="password", key="cl_com")
...
def modulo_espino():
    ...
    with t2:
        ...
        if es_admin and not df_h.empty:
            ...
            n_it = st.text_input("Modificar", item_e['item']); n_mt = st.number_input("Monto", value=float(item_e['monto']))
            if st.button("✏️ ACTUALIZAR") and st.text_input("Master", type="password", key="cl_e") == CLAVE_MAESTRA:
                conn.execute("UPDATE gastos_espino SET item=?, monto=? WHERE id=?", (n_it, n_mt, id_e)); conn.commit(); st.rerun()
            if st.button("🗑️ ELIMINAR") and st.text_input("Master ", type="password", key="cl_del_e") == CLAVE_MAESTRA:
...
def modulo_tesoreria():
    ...
    with t2:
        ...
        if not df_prv.empty:
            prv = st.selectbox("Proveedor", df_prv['proveedor'])
...
def modulo_bodega():
    ...
    with t1:
        ...
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl':
            st.divider(); id_b = st.selectbox("ID Insumo", df_s['id']); item = df_s[df_s['id']==id_b].iloc[0]
            n_nom = st.text_input("Nombre", item['producto']); n_st = st.number_input("Stock", value=float(item['stock']))
            if st.button("✏️ MODIFICAR") and st.text_input("Clave Master", type="password", key="cl_b") == CLAVE_MAESTRA:
