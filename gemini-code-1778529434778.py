...
def inicializar_db():
    ...
    # Facturas: nro, proveedor, fechas, montos, estado, tipo, metodo, pago, concepto, centro_costo, monto_imputado
    ...
    # Inventario: producto, familia, stock, stock_minimo, precio_medio
    ...
    # Movimientos: id_prod, tipo, cantidad, centro_costo, fecha, valor_imputado
    ...
    # Migración de Columnas para asegurar v7.6
...
def guardar_en_drive():
    try:
        ...
        st.success("✅ Sincronizado en la Nube")
        return True
    except: return False
...
def descargar_pdf(df, titulo, es_valorizado=False):
    try:
        ...
        for _, row in df.iterrows():
            ...
            for i, item in enumerate(row):
                ...
                if isinstance(item, (int, float)):
                    if any(x in col_name for x in ["cantidad", "stock", "precio_medio", "valor_imputado", "imputado"]):
        ...
        pdf.set_font("Helvetica", "B", 9); pdf.cell(w * (len(cols)-1), 8, "VALOR TOTAL REPORTE:", border=1, align="R")
...
def modulo_dashboard():
    st.header("📊 Dashboard Maestro de Gestión")
    conn = conectar_db()
    ...
    c3.metric("Docs. Vencidos (Hoy)", vencidos_hoy, delta="Alerta", delta_color="inverse")
    c4.metric("Total Pendientes", len(df_f))

    if atrasado > 0: st.error(f"🚨 Tienes ${f_puntos(atrasado)} en facturas de meses anteriores sin pagar.")

    st.markdown("---")
    col_izq, col_der = st.columns([1, 1])
    
    with col_izq:
        st.subheader("📅 Proyección de Flujo (Próx. 4 meses)")
        cols_m = st.columns(4); meses_n = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"]
        for i in range(4):
            ...
            cols_m[i].metric(f"{meses_n[m-1]} {a}", f"${f_puntos(val)}")
            
    with col_der:
        st.subheader("💰 Resumen Costos por Cuartel (Neto)")
...
def modulo_compras():
    ...
    with t1:
        ...
        if st.session_state.get('car'):
            df_car = pd.DataFrame(st.session_state['car']); st.table(df_car); total_fac = st.number_input("Monto Total Factura (IVA)", value=float(df_car['t'].sum()*1.19))
            if st.button("💾 GUARDAR FACTURA"):
                conn = conectar_db(); cur = conn.cursor()
                cur.execute("INSERT INTO facturas (nro_documento, proveedor, fecha_compra, fecha_vencimiento, monto_total) VALUES (?,?,?,?,?)", (nro, prov, fe, fv, total_fac))
    ...
    with t2:
        with st.form("gv"):
            ...
            cc = st.selectbox("Asignar a Cuartel (CC)", CENTROS_COSTO); iva = st.radio("¿IVA al Costo?", ["SÍ (Monto Total)", "NO (Solo Neto)"])
            if st.form_submit_button("💾 GUARDAR GASTO VARIO"):
                m_imp = m if iva == "SÍ (Monto Total)" else m / 1.19
    ...
    with t3:
        ...
        cf1, cf2 = st.columns(2); h1, h2 = cf1.date_input("Compra Desde", fmin), cf2.date_input("Compra Hasta", fmax)
        ...
        if not df_h.empty:
            st.download_button("📥 PDF Historial", descargar_pdf(df_h, "HISTORIAL DE COMPRAS"), "historial.pdf")
            ...
            with st.expander("📝 MODIFICAR / 🗑️ ELIMINAR"):
                ...
                cl = st.text_input("Clave de Seguridad", type="password", key="mcomp")
                ...
                if cb2.button("BORRAR DEFINITIVO") and cl == CLAVE_SEGURIDAD:
...
def modulo_tesoreria():
    ...
    with tp2:
        ...
        if not df_pr.empty:
            p_sel = st.selectbox("Proveedor", df_pr['proveedor'])
            ...
            st.download_button(f"📥 PDF {p_sel}", descargar_pdf(df_f, f"DEUDA: {p_sel}"), f"deuda_{p_sel}.pdf")
    with tp3:
        f1, f2 = st.date_input("Desde Vencimiento", hoy), st.date_input("Hasta Vencimiento", hoy + timedelta(days=30))
        ...
        if not df_r.empty:
            st.dataframe(df_r.style.apply(cv, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True)
            st.download_button("📥 PDF Rango Selección", descargar_pdf(df_r, "VENCIMIENTOS POR RANGO"), "rango_venc.pdf")
    conn.close()

def modulo_bodega():
    st.header("🚜 Gestión de Bodega")
    tb1, tb2, tb3, tb4 = st.tabs(["📊 Stock Actual", "🔄 Movimientos", "➕ Nuevo Insumo", "🔍 Consulta por Cuartel"])
    ...
    with tb1:
        ...
        if not df_s.empty:
            c1, c2 = st.columns(2)
            c1.download_button("📥 PDF Stock (Trabajadores)", descargar_pdf(df_s.drop(columns=['precio_medio']), "LISTADO STOCK"), "stock_campo.pdf")
            c2.download_button("💰 PDF Valorización (Administración)", descargar_pdf(df_s, "VALORIZACIÓN BODEGA", es_valorizado=True), "stock_admin.pdf")
            st.divider(); st.subheader("⚙️ Gestión de Catálogo")
            id_ins = st.selectbox("ID Insumo", df_s['id']); row_i = df_s[df_s['id'] == id_ins].iloc[0]
            mi_nom = st.text_input("Editar Nombre", row_i['producto']); mi_pmp = st.number_input("Ajustar PMP Neto", value=float(row_i['precio_medio'])); cl = st.text_input("Clave", type="password", key="mbod")
            cb1, cb2 = st.columns(2)
            if cb1.button("ACTUALIZAR") and cl == CLAVE_SEGURIDAD: conn.execute("UPDATE inventario SET producto=?, precio_medio=? WHERE id=?", (mi_nom, mi_pmp, id_ins)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
            if cb2.button("ELIMINAR") and cl == CLAVE_SEGURIDAD: conn.execute("DELETE FROM inventario WHERE id=?", (id_ins,)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
        conn.close()
    ...
    with tb2:
        ...
        with st.form("mov"):
            ...
            ps = st.selectbox("Insumo", df_i['id'].astype(str) + " - " + df_i['producto']); ct, cc = st.number_input("Cantidad", min_value=0.1), st.selectbox("CC Destino", CENTROS_COSTO)
            if st.form_submit_button("REGISTRAR MOVIMIENTO"):
                item = df_i[df_i['id'] == int(ps.split(" - ")[0])].iloc[0]; valor_imp = ct * item['precio_medio'] if "Salida" in tipo else 0
                conn = conectar_db(); cur = conn.cursor()
    ...
    with tb3:
        with st.form("ni"):
            ...
            if st.form_submit_button("💾 CREAR PRODUCTO"): conn = conectar_db(); conn.execute("INSERT INTO inventario (producto, familia, stock) VALUES (?,?,0)", (n, f)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
    ...
    with tb4:
        ...
        if not df_cc.empty: st.dataframe(df_cc, use_container_width=True); st.download_button(f"📥 PDF {cc_sel}", descargar_pdf(df_cc, f"MOVIMIENTOS: {cc_sel}"), f"mov_{cc_sel}.pdf")

def modulo_costos():
    st.header("💰 Gestión de Costos Consolidados")
    conn = conectar_db()
    df_i = pd.read_sql_query("SELECT centro_costo, SUM(valor_imputado) as total_insumos FROM movimientos WHERE tipo LIKE 'Salida%' GROUP BY centro_costo", conn)
    df_g = pd.read_sql_query("SELECT centro_costo, SUM(monto_imputado) as total_gastos FROM facturas WHERE tipo='Gasto Vario' GROUP BY centro_costo", conn)
    conn.close()
    df_t = pd.merge(df_i, df_g, on='centro_costo', how='outer').fillna(0)
    df_t['COSTO NETO TOTAL'] = df_t['total_insumos'] + df_total_gastos = df_t['total_gastos'] # Se corrigió error de variable
    df_t['COSTO NETO TOTAL'] = df_t['total_insumos'] + df_t['total_gastos']
    st.dataframe(df_t.style.format({"total_insumos": "${:,.0f}", "total_gastos": "${:,.0f}", "COSTO NETO TOTAL": "${:,.0f}"}), use_container_width=True)
    if not df_t.empty: st.download_button("📥 PDF Costos", descargar_pdf(df_t, "REPORTE DE COSTOS"), "costos.pdf")

# --- 5. NAVEGACIÓN ---
st.set_page_config(page_title="LA CONCEPCIÓN ERP v7.6", page_icon="🚜", layout="wide")
...
with st.sidebar:
    ...
    menu = st.radio("Módulos de Gestión", ["🏠 Dashboard", "📦 Compras", "💸 Tesorería", "🚜 Bodega", "💰 COSTOS"])
    st.divider()
