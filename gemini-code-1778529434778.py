...
def generar_pdf_blob(df, titulo, incluir_precios=True, total_manual=None):
    try:
        ...
        total_real = total_manual
        if total_real is None and 'Total' in df_pdf.columns: total_real = df_pdf['Total'].sum()
        ...
        for _, row in df_pdf.iterrows():
            ...
            pdf.ln()
        if incluir_precios and total_real is not None:
            pdf.set_font("Helvetica", "B", 9); pdf.cell(w * (len(cols)-1), 8, "TOTAL FINAL (INC. AJUSTES):", border=1, align="R")
            pdf.cell(w, 8, f"${f_puntos(total_real)}", border=1, align="L")
...
def modulo_dashboard():
    ...
    venc_count = len(df_f[pd.to_datetime(df_f['fecha_vencimiento']).dt.date < hoy])
    with m3: st.markdown(f"<div class='stMetric card-vencida'><small>⚠️ VENCIDAS</small><br><span class='metric-blue'>{venc_count}</span><br><small style='color:#1976d2;'>DOCUMENTOS</small></div>", unsafe_allow_html=True)
...
def modulo_compras():
    ...
    with t1:
        ...
        ct, pr = cp2.number_input("Cant", 0.0), cp3.number_input("Neto Unit.", 0.0)
        if cp4.button("➕"):
...
def modulo_tesoreria():
    ...
    with t1:
        ...
        # BOTÓN PDF PENDIENTES RESTAURADO
...
def modulo_bodega():
    ...
    with t1:
        ...
        cl = st.text_input("Master", type="password", key="cl_b")
        if st.button("✏️ MODIFICAR") and cl == CLAVE_MAESTRA: conn.execute("UPDATE inventario SET producto=?, stock=? WHERE id=?", (n_nom, round(n_st, 2), id_g)); conn.commit(); st.rerun()
        if st.button("🗑️ ELIMINAR") and cl == CLAVE_MAESTRA: conn.execute("DELETE FROM inventario WHERE id=?", (id_g,)); conn.commit(); st.rerun()
    with t2:
    ...
    with t3:
        with st.form("nuevo_form"):
            n_p = st.text_input("Nombre Insumo"); n_f = st.selectbox("Familia", FAMILIAS_PRODUCTOS); n_s = st.number_input("Stock Inicial", 0.0); n_pn = st.number_input("PMP Neto", 0.0)
    ...
    with t4:
        cc_q = st.selectbox("Cuartel a consultar", CENTROS_COSTO)
        ...
        # BOTÓN PDF CONSULTA POR CC (SOLICITADO)
...
def modulo_costos():
    ...
    with t[0]:
        ...
        if not df_r.empty:
            df_m = df_r.copy(); total_real = df_m['Total'].sum()
            if not es_admin: df_m = df_m.drop(columns=['Ajustes'])
            # Gastos sin decimales
            ...
            # PDF con sumatoria real
            st.download_button("📥 PDF Costos (Sin Ajustes)", generar_pdf_blob(df_r, "INFORME COSTOS POR CUARTEL", total_manual=total_real), "costos.pdf")
    if es_admin:
        with t[1]:
            cc_aj, m_aj, mot = st.selectbox("CC", CENTROS_COSTO), st.number_input("Monto"), st.text_input("Motivo")
            if st.button("💾 APLICAR") and st.text_input("Clave Master", type="password") == CLAVE_MAESTRA: conn.execute("INSERT INTO ajustes_costos (centro_costo, monto, fecha, motivo) VALUES (?,?,?,?)", (cc_aj.upper(), m_aj, hoy, mot)); conn.commit(); st.rerun()
    conn.close()
...
def modulo_seguridad():
    ...
    conn = conectar_db()
    t1, t2 = st.tabs(["📜 Bitácora", "🔑 Accesos Usuarios"]); 
    with t1:
    ...
    with col2:
        with st.form("login"):
            ...
            if st.form_submit_button("ACCEDER"):
                ...
                else: conn.close(); st.error("Error")

# --- NAVEGACIÓN ---
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.69", layout="wide")
inicializar_db()
