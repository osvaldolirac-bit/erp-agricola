...
def modulo_dashboard():
    ...
    c3.metric("Vencidos Hoy", vencidos_hoy_count)
    if vencidos_hoy_count > 0:
        c3.error("⚠️ ALERTA DE VENCIMIENTO")
...
def modulo_tesoreria():
    ...
    with tp1:
        # Orden de la más antigua a la más nueva
        ...
        st.subheader(f"💰 TOTAL DEUDA PENDIENTE: ${f_puntos(deuda_t)}")
    ...
    with tp2:
        ...
        if not df_pr.empty:
            ...
            deuda_p = df_f['monto_total'].sum() if not df_f.empty else 0
            st.subheader(f"🏢 DEUDA CON {p_sel}: ${f_puntos(deuda_p)}")
    ...
    with tp3:
        ...
        deuda_r = df_r['monto_total'].sum() if not df_r.empty else 0
        st.subheader(f"📅 DEUDA EN RANGO SELECCIONADO: ${f_puntos(deuda_r)}")
        if not df_r.empty:
            st.dataframe(df_r.style.format({"monto_total": "${:,.0f}"}), use_container_width=True)
            st.download_button("📥 PDF Rango", generar_pdf_blob(df_r, "VENCIMIENTOS RANGO"), "venc_rango.pdf")
    conn.close()
...
# --- 5. NAVEGACIÓN ---
st.set_page_config(page_title="LA CONCEPCIÓN ERP v8.4", layout="wide")
