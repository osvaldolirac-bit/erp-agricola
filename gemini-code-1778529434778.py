...
def guardar_en_drive():
    try:
        ...
        st.success("✅ Sincronizado en Cloud")
        return True
    except: return False
...
def modulo_dashboard():
    ...
    vencidos_hoy = len(df_f[pd.to_datetime(df_f['fecha_vencimiento']).dt.date < hoy]) if not df_f.empty else 0
    ...
    c3.metric("Docs. Vencidos", vencidos_hoy, delta="Revisar", delta_color="inverse")
    c4.metric("Pendientes", len(df_f))

    if atrasado > 0: st.error(f"🚨 Tienes ${f_puntos(atrasado)} de meses anteriores.")
    ...
    with col_izq:
        ...
        for i in range(4):
            ft = (datetime.now().replace(day=1) + timedelta(days=i*31)).replace(day=1); m, a = ft.month, ft.year
    ...
    with col_der:
        st.subheader("💰 Resumen Costos por CC")
...
def modulo_compras():
    ...
    with t2:
        with st.form("gv"):
            ...
            cc = st.selectbox("Asignar a CC", CENTROS_COSTO); iva = st.radio("¿IVA al Costo?", ["SÍ (Total)", "NO (Neto)"])
    ...
    with t3:
        ...
        fmin = pd.to_datetime(rf[0]).date() if rf[0] else hoy - timedelta(days=365); fmax = pd.to_datetime(rf[1]).date() if rf[1] else hoy; cf1, cf2 = st.columns(2); h1, h2 = cf1.date_input("Desde", fmin), cf2.date_input("Hasta", fmax)
        ...
        if not df_h.empty:
            ...
            with st.expander("📝 MODIFICAR / 🗑️ ELIMINAR"):
                ...
                if colb1.button("ACTUALIZAR") and cl == CLAVE_SEGURIDAD: conn.execute("UPDATE facturas SET nro_documento=?, monto_total=? WHERE id=?", (mn, mm, id_sel)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
                if colb2.button("ELIMINAR") and cl == CLAVE_SEGURIDAD: conn.execute("DELETE FROM facturas WHERE id=?", (id_sel,)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
        conn.close()

def modulo_tesoreria():
    st.header("💸 Tesorería")
    tp1, tp2, tp3 = st.tabs(["🔴 Pendientes General", "🏢 Por Proveedor", "📅 Por Rango Vencimiento"])
    conn = conectar_db()
    ...
    with tp2:
        ...
        if not df_pr.empty:
            p_sel = st.selectbox("Seleccione Proveedor", df_pr['proveedor']); df_filt = pd.read_sql_query(f"SELECT nro_documento, fecha_vencimiento, monto_total FROM facturas WHERE estado='Pendiente' AND proveedor='{p_sel}'", conn); st.dataframe(df_filt.style.apply(cv, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True); st.download_button(f"📥 PDF {p_sel}", descargar_pdf(df_filt, f"DEUDA: {p_sel}"), f"pago_{p_sel}.pdf")
    with tp3:
        ...
        if not df_r.empty: st.dataframe(df_r.style.apply(cv, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True); st.download_button("📥 PDF Rango", descargar_pdf(df_r, "VENCIMIENTOS RANGO"), "rango.pdf")
    conn.close()
...
def modulo_bodega():
    ...
    with tb1:
        ...
        if not df_s.empty:
            ...
            st.divider(); id_ins = st.selectbox("ID Insumo", df_s['id']); row_i = df_s[df_s['id'] == id_ins].iloc[0]; mi_nom = st.text_input("Editar Nombre", row_i['producto']); mi_pmp = st.number_input("Precio PMP", value=float(row_i['precio_medio'])); cl = st.text_input("Clave", type="password", key="mbod")
    ...
    with tb2:
        ...
        with st.form("mov"):
            ...
            ps = st.selectbox("Insumo", df_i['id'].astype(str) + " - " + df_i['producto']); ct, cc = st.number_input("Cantidad", min_value=0.1), st.selectbox("CC", CENTROS_COSTO)
    ...
    with tb3:
        with st.form("ni"):
            ...
            if st.form_submit_button("💾 CREAR"): conn = conectar_db(); conn.execute("INSERT INTO inventario (producto, familia, stock) VALUES (?,?,0)", (n, f)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
    with tb4:
        cc_sel = st.selectbox("Cuartel", CENTROS_COSTO); conn = conectar_db(); df_cc = pd.read_sql_query(f"SELECT m.fecha, i.producto, m.tipo, m.cantidad, m.valor_imputado FROM movimientos m JOIN inventario i ON m.producto_id = i.id WHERE m.centro_costo = '{cc_sel}' ORDER BY m.fecha DESC", conn); conn.close()
...
def modulo_costos():
    ...
    conn = conectar_db(); df_ins = pd.read_sql_query("SELECT centro_costo, SUM(valor_imputado) as total_insumos FROM movimientos WHERE tipo LIKE 'Salida%' GROUP BY centro_costo", conn); df_gas = pd.read_sql_query("SELECT centro_costo, SUM(monto_imputado) as total_gastos FROM facturas WHERE tipo='Gasto Vario' GROUP BY centro_costo", conn); conn.close()
    df_total = pd.merge(df_ins, df_gas, on='centro_costo', how='outer').fillna(0); df_total['TOTAL CC'] = df_total['total_insumos'] + df_total['total_gastos']
    st.dataframe(df_total.style.format({"total_insumos": "${:,.0f}", "total_gastos": "${:,.0f}", "TOTAL CC": "${:,.0f}"}), use_container_width=True)
    if not df_total.empty: st.download_button("📥 PDF Costos", descargar_pdf(df_total, "REPORTE DE COSTOS"), "costos.pdf")

# --- 5. NAVEGACIÓN ---
st.set_page_config(page_title="LA CONCEPCIÓN ERP v7.5", page_icon="🚜", layout="wide")
