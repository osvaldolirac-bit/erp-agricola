...
def generar_pdf_blob(df, titulo, incluir_precios=True, total_manual=None, modo_petroleo=False):
    try:
        ...
        t_sum = total_manual if total_manual is not None else (df_pdf['Total'].sum() if 'Total' in df_pdf.columns else None)

        if modo_petroleo:
            # Reporte operativo: Elimina valores monetarios
        ...
        for _, row in df_pdf.iterrows():
            for i, item in enumerate(row):
                ...
                if any(x in col_n for x in ["gastos", "insumos", "petroleo", "total"]): val = f_puntos(item)
...
def modulo_dashboard():
    ...
    with m4: st.markdown(f"<div class='stMetric'><small>📄 PENDIENTES</small><br><span class='metric-std'>{len(df_f)}</span><br><small>POR PAGAR</small></div>", unsafe_allow_html=True)
...
def modulo_petroleo():
    ...
    with t1:
        with st.form("form_carga"):
            lts, mt, f = st.number_input("Litros", 0.0), st.number_input("Total Bruto ($)", 0.0), st.date_input("Fecha", hoy)
    ...
    with t2:
        with st.form("form_salida"):
            lts_s, veh, res = st.number_input("Litros Salida", 0.0), st.text_input("Vehículo"), st.text_input("Responsable")
            ccs_sel = []
            cols = st.columns(3)
            for cc in CENTROS_COSTO:
                if st.checkbox(cc, key=f"ps_{cc}"): ccs_sel.append(cc)
            if st.form_submit_button("REGISTRAR SALIDA"):
                ...
                for c in ccs_sel:
                    conn.execute("INSERT INTO petroleo (tipo, litros, vehiculo, responsable, centro_costo, fecha, valor_imputado) VALUES (?,?,?,?,?,?,?)", 
                                 ("Salida", lts_s/len(ccs_sel), veh, res, c.upper(), hoy, (lts_s/len(ccs_sel)*pmp)))
                conn.commit(); registrar_accion("SALIDA PETROLEO", f"{lts_s}L para {veh}"); st.rerun()
    with t3:
        ...
        # PDF: Operativo (Sin Valor Imputado)
        st.download_button("📥 PDF Historial Operativo", generar_pdf_blob(df_p, "HISTORIAL PETROLEO (OPERATIVO)", modo_petroleo=True), "petroleo_operativo.pdf")
        id_del = st.selectbox("ID a borrar", df_p['id']); cl = st.text_input("Master", type="password", key="cl_p")
        if st.button("🗑️ ELIMINAR") and cl == CLAVE_MAESTRA: conn.execute("DELETE FROM petroleo WHERE id=?", (id_del,)); conn.commit(); st.rerun()
    conn.close()
...
def modulo_compras():
    ...
    with t2:
        ...
        sel_cc = []
        for cc in CENTROS_COSTO:
            if st.checkbox(cc, key=f"gv_{cc}"): sel_cc.append(cc)
    ...
    with t3:
        ...
        st.download_button("📥 PDF Historial", generar_pdf_blob(df_h, "HISTORIAL DE COMPRAS"), "compras.pdf")
...
def modulo_tesoreria():
    ...
    with t1:
        ...
        # RESTAURADO: Botón PDF Pendientes
        st.download_button("📥 PDF Pendientes", generar_pdf_blob(df_p.drop(columns=['id']), "LISTADO DE PENDIENTES"), "pendientes.pdf")
...
def modulo_bodega():
    ...
    with t2:
        ...
        ps = st.selectbox("Insumo", df_i['id'].astype(str) + " - " + df_i['producto']); ct = st.number_input("Cantidad Salida", 0.0)
        sel_cc = []
        for cc in CENTROS_COSTO:
            if st.checkbox(cc, key=f"mb_{cc}"): sel_cc.append(cc)
        if st.button("REGISTRAR SALIDA"):
            iid = int(ps.split(" - ")[0]); pmp = df_i[df_i['id']==iid]['precio_medio'].iloc[0]
            if ct > 0 and sel_cc:
                c_por_cc = round(ct/len(sel_cc), 2)
                for c in sel_cc: conn.execute("INSERT INTO movimientos (producto_id, tipo, cantidad, fecha, centro_costo, valor_imputado) VALUES (?,?,?,?,?)", (iid, "Salida", c_por_cc, hoy, c.upper(), (c_por_cc*pmp)))
                conn.execute("UPDATE inventario SET stock = stock - ? WHERE id = ?", (round(ct, 2), iid)); conn.commit(); registrar_accion("SALIDA BODEGA", f"{ct} de {ps}"); st.rerun()
    with t3:
        with st.form("nuevo_insumo"):
            n_p, n_f = st.text_input("Nombre"), st.selectbox("Familia", FAMILIAS_PRODUCTOS)
            n_s, n_pn = st.number_input("Stock Inicial", 0.0), st.number_input("Precio Neto", 0.0)
    ...
    with t4:
        cc_q = st.selectbox("Seleccione Cuartel", CENTROS_COSTO)
        ...
        # RESTAURADO: Botón PDF Consulta por CC
...
def modulo_costos():
    ...
    with t[0]:
        ...
        if not df_r.empty:
            df_m = df_r.copy(); t_sum = df_m['Total'].sum()
            if not es_admin: df_m = df_m.drop(columns=['Ajustes'])
            # Columna Gastos sin decimales
            ...
            st.download_button("📥 PDF Costos", generar_pdf_blob(df_r, "INFORME DE COSTOS", total_manual=t_sum), "costos.pdf")
    if es_admin:
        with t[1]:
            cc_aj, m_aj, mot = st.selectbox("CC Ajuste", CENTROS_COSTO), st.number_input("Monto"), st.text_input("Motivo")
            if st.button("💾 APLICAR") and st.text_input("Master", type="password") == CLAVE_MAESTRA: conn.execute("INSERT INTO ajustes_costos (centro_costo, monto, fecha, motivo) VALUES (?,?,?,?)", (cc_aj.upper(), m_aj, hoy, mot)); conn.commit(); st.rerun()
    conn.close()
...
def modulo_seguridad():
    ...
    with t1:
        ...
        st.download_button("📥 PDF Bitácora", generar_pdf_blob(df_b, "BITACORA DE MOVIMIENTOS"), "bitacora.pdf")
    with t2:
        df_a = pd.read_sql_query("SELECT * FROM log_accesos ORDER BY id DESC", conn); st.dataframe(df_a, use_container_width=True)
        st.download_button("📥 PDF Accesos", generar_pdf_blob(df_a, "HISTORIAL DE ACCESOS"), "accesos.pdf")
    conn.close()
...
# --- NAVEGACIÓN ---
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.72", layout="wide")
inicializar_db()
...
if not st.session_state['logged_in']: login_page()
    ...
    with st.sidebar:
        ...
        sel = st.radio("MENÚ", list(m_opts.keys()))
        menu = m_opts[sel]
        st.divider()
