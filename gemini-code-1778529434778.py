...
def guardar_en_drive():
    ...
    if drive:
        try:
            # Buscar si el archivo ya existe para sobreescribirlo
            query = f"'{ID_CARPETA_DRIVE}' in parents and title='{NOMBRE_DB}' and trashed=false"
            lista = drive.ListFile({'q': query}).GetList()
            
            if lista:
                f = lista[0] # Usar el archivo existente
            else:
            ...
            st.error(f"❌ Error de permisos en Drive: Asegúrese de que la carpeta esté compartida con el robot. Detalles: {e}")
    else:
        st.error("❌ No hay conexión con Drive configurada.")
...
def modulo_dashboard():
    ...
    t_d, v_a = (df_f['monto_total'].sum() if not df_f.empty else 0), (df_f[pd.to_datetime(df_f['fecha_vencimiento']).dt.date < hoy.replace(day=1)]['monto_total'].sum() if not df_f.empty else 0)
    ...
    with col1:
        ...
        if not df_c.empty:
            ...
            st.dataframe(df_res.format({"total_neto": "${:,.0f}"}), use_container_width=True)
    with col2:
...
def modulo_compras():
    ...
    with t3:
        ...
        st.dataframe(df_h.format({"monto_total": "${:,.0f}"}), use_container_width=True)
        if not df_h.empty:
...
def modulo_bodega():
    ...
    with tb1:
        ...
        st.dataframe(df_s.format({"stock": "{:,.2f}", "precio_medio": "${:,.0f}"}), use_container_width=True)
        if not df_s.empty:
    ...
    with tb4:
        ...
        if not df_cc.empty: 
            st.dataframe(df_cc.format({"cantidad": "{:,.2f}", "valor_imputado": "${:,.0f}"}), use_container_width=True)
            st.download_button("📥 PDF Movimientos", generar_pdf_blob(df_cc, f"MOVIMIENTOS: {cc_sel}"), f"mov_{cc_sel}.pdf")
    conn.close()

# --- NAVEGACIÓN ---
st.set_page_config(page_title="ERP LA CONCEPCIÓN v11.1", layout="wide")
inicializar_db()
