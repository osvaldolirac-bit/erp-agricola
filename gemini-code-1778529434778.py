...
def inicializar_db():
    ...
    # Migración de columnas para integridad v7.8
...
def descargar_pdf(df, titulo, es_valorizado=False):
    try:
        ...
        pdf.cell(w, 8, f"${f_puntos(total_gral)}", border=1, align="L")
...
def modulo_dashboard():
    ...
    if atrasado > 0: st.error(f"🚨 Tienes ${f_puntos(atrasado)} en facturas que vencieron en meses anteriores.")
    ...
    with col2:
        st.subheader("💰 Resumen Costos Neto por CC")
...
def modulo_compras():
    ...
    with t2:
        with st.form("gv"):
            p, n, m = st.text_input("Proveedor"), st.text_input("N° Doc"), st.number_input("Monto Total"); cc = st.selectbox("Asignar a CC", CENTROS_COSTO); iva = st.radio("¿IVA al Costo?", ["SÍ (Total)", "NO (Neto)"])
    ...
    with t3:
        conn = conectar_db(); rf = conn.execute("SELECT MIN(fecha_compra), MAX(fecha_compra) FROM facturas").fetchone(); fmin = pd.to_datetime(rf[0]).date() if rf[0] else hoy-timedelta(days=365); fmax = pd.to_datetime(rf[1]).date() if rf[1] else hoy; cf1, cf2 = st.columns(2); h1, h2 = cf1.date_input("Desde", fmin), cf2.date_input("Hasta", fmax)
        df_h = pd.read_sql_query(f"SELECT id, nro_documento, proveedor, fecha_compra, monto_total, tipo FROM facturas WHERE fecha_compra BETWEEN '{h1}' AND '{h2}' ORDER BY fecha_compra DESC", conn); st.dataframe(df_h, use_container_width=True)
        if not df_h.empty:
            ...
            with st.expander("📝 MODIFICAR / ELIMINAR"):
                mn, mm = st.text_input("Nuevo N°", row['nro_documento']), st.number_input("Nuevo Monto Total", value=float(row['monto_total'])); cl = st.text_input("Clave", type="password", key="mcomp")
                c_b1, c_b2 = st.columns(2)
                if c_b1.button("ACTUALIZAR") and cl == CLAVE_SEGURIDAD: conn.execute("UPDATE facturas SET nro_documento=?, monto_total=? WHERE id=?", (mn, mm, id_sel)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
                if c_b2.button("BORRAR") and cl == CLAVE_SEGURIDAD: conn.execute("DELETE FROM facturas WHERE id=?", (id_sel,)); conn.commit(); conn.close(); guardar_en_drive(); st.rerun()
        conn.close()

def modulo_tesoreria():
    st.header("💸 Tesorería")
    tp1, tp2, tp3 = st.tabs(["🔴 Pendientes General", "🏢 Por Proveedor", "📅 Por Vencimiento"])
    ...
    with tp1:
        ...
        if not df_p.empty:
            st.dataframe(df_p.style.apply(cv, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True); st.metric("Deuda Total", f"${f_puntos(df_p['monto_total'].sum())}"); st.download_button("📥 PDF Pendientes", descargar_pdf(df_p, "PENDIENTES GENERALES"), "pendientes.pdf")
    ...
    with tp2:
        ...
        if not df_pr.empty:
            p_s = st.selectbox("Proveedor", df_pr['proveedor']); df_filt = pd.read_sql_query(f"SELECT nro_documento, fecha_vencimiento, monto_total FROM facturas WHERE estado='Pendiente' AND proveedor='{p_s}'", conn); st.dataframe(df_filt.style.apply(cv, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True); st.download_button("📥 PDF Deuda Proveedor", descargar_pdf(df_filt, f"DEUDA: {p_s}"), "prov.pdf")
    with tp3:
        ...
        if not df_r.empty: st.dataframe(df_r.style.apply(cv, axis=1).format({"monto_total": "${:,.0f}"}), use_container_width=True); st.download_button("📥 PDF Rango", descargar_pdf(df_r, "VENCIMIENTOS POR RANGO"), "rango.pdf")
    conn.close()

def modulo_bodega():
    st.header("🚜 Gestión de Bodega")
    tb1, tb2, tb3, tb4 = st.tabs(["📊 Stock Actual", "🔄 Movimientos", "➕ Nuevo Insumo", "🔍 Consulta por CC"])
    with tb1:
        ...
        if not df_s.empty:
            c1, c2 = st.columns(2)
            c1.download_button("📥 PDF Trabajadores", descargar_pdf(df_s.drop(columns=['precio_medio']), "LISTADO DE STOCK"), "stock_campo.pdf")
            c2.download_button("💰 PDF Administración (Valorizado)", descargar_pdf(df_s, "VALORIZACIÓN DE BODEGA", es_valorizado=True), "stock_admin.pdf")
            st.divider(); id_ins = st.selectbox("ID Insumo", df_s['id']); row_i = df_s[df_s['id'] == id_ins].iloc[0]; mi_nom = st.text_input("Nombre", row_i['producto']); mi_pmp = st.number_input("Precio PMP Neto", value=float(row_i['precio_medio'])); cl = st.text_input("Clave", type="password", key="mbod")
    ...
    with tb4:
        cc_sel = st.selectbox("Consultar CC", CENTROS_COSTO); conn = conectar_db(); df_cc = pd.read_sql_query(f"SELECT m.fecha, i.producto, m.tipo, m.cantidad, m.valor_imputado FROM movimientos m JOIN inventario i ON m.producto_id = i.id WHERE m.centro_costo = '{cc_sel}' ORDER BY m.fecha DESC", conn); conn.close()
...
def modulo_costos():
    ...
    df_t = pd.merge(df_i, df_g, on='centro_costo', how='outer').fillna(0); df_t['TOTAL NETO'] = df_t['insumos'] + df_t['gastos']
    st.dataframe(df_t.style.format({"insumos": "${:,.0f}", "gastos": "${:,.0f}", "TOTAL NETO": "${:,.0f}"}), use_container_width=True)
    if not df_t.empty: st.download_button("📥 PDF Reporte Costos", descargar_pdf(df_t, "REPORTE CONSOLIDADO COSTOS"), "costos.pdf")

# --- 5. NAVEGACIÓN ---
st.set_page_config(page_title="LA CONCEPCIÓN ERP v7.7", page_icon="🚜", layout="wide")
