...
# ASEGÚRATE QUE ESTE ID SEA EL CORRECTO (sacado de la URL de tu carpeta de Drive)
...
def guardar_en_drive():
    ...
    try:
        # Intentamos buscar el archivo existente en la carpeta específica
        ...
        if lista:
            ...
            f = lista[0]
            st.info(f"🔄 Actualizando archivo existente (ID: {f['id']})...")
        else:
            # Si no existe, lo creamos nuevo en esa carpeta
            st.info("🆕 Creando nuevo respaldo en Drive...")
            f = drive.CreateFile({
                'title': NOMBRE_DB,
                'parents': [{'id': ID_CARPETA_DRIVE}]
            })
        
        # Subida del archivo física
        f.SetContentFile(NOMBRE_DB)
        f.Upload()
        st.success("✅ ¡Sincronización Exitosa! La base de datos está respaldada.")
        
    except Exception as e:
        # Si falla por el ID de carpeta, intentamos en la raíz del robot para no fallar
        st.warning("⚠️ Falló la carpeta específica. Intentando guardar en la raíz del Drive...")
        try:
            f_root = drive.CreateFile({'title': NOMBRE_DB + "_ROOT_BACKUP"})
            f_root.SetContentFile(NOMBRE_DB)
            f_root.Upload()
            st.success("✅ Respaldo guardado en la raíz (revisa el ID de carpeta en el código).")
        except Exception as e2:
            st.error(f"❌ Error de API Google: {str(e2)}")
...
# --- NAVEGACIÓN ---
st.set_page_config(page_title="ERP LA CONCEPCIÓN v11.2", layout="wide")
inicializar_db()
if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False
if not st.session_state['logged_in']: login_page()
else:
    # Intento de descarga inicial silencioso
    ...
    with st.sidebar:
        st.title("MENÚ")
        d_conn = obtener_drive()
        if d_conn: st.markdown("🟢 **Drive: CONECTADO**")
