...
def sanear_base_datos():
    """Limpieza de registros duplicados huérfanos (v10.8.53)"""
    try:
        conn = conectar_db()
...
def modulo_dashboard():
    ...
    with c2: st.markdown(f"<div class='custom-metric'><span class='metric-label'>MESES ANTERIORES</span><span class='metric-value' style='color:red;'>${f_puntos(v_a)}</span></div>", unsafe_allow_html=True)
...
def modulo_petroleo():
    ...
    with tp3:
        ...
        id_p = st.selectbox("ID a borrar", df_p['id']); cl = st.text_input("Master", type="password", key="p_cl")
...
def modulo_compras():
    ...
    with t3:
        df_h = pd.read_sql_query("SELECT id, nro_documento, proveedor, fecha_compra, monto_total, estado FROM facturas WHERE monto_total > 0 AND nro_documento NOT LIKE '%_P' ORDER BY fecha_compra DESC", conn)
...
def modulo_bodega():
    st.header("🚜 Bodega")
    tb1, tb2, tb3 = st.tabs(["📊 Stock Actual", "🔄 Salida", "➕ Nuevo"]); conn = conectar_db()
    with tb1:
        ...
        cl = st.text_input("Master", type="password", key="cl_bod")
...
def modulo_costos():
    ...
    tabs_nombres = ["📊 Resumen"]
    if es_admin: tabs_nombres.append("🔧 AJUSTE DE SALDOS")
    tabs = st.tabs(tabs_nombres); conn = conectar_db()
    with tabs[0]:
        ...
        if not df_r.empty:
            if not es_admin: df_r = df_r.drop(columns=['Ajustes'])
            st.dataframe(df_r.style.format({c: "${:,.0f}" for c in df_r.columns if c != 'cc'}), use_container_width=True)
    if es_admin:
        with tabs[1]:
            cc_aj = st.selectbox("Cuartel", CENTROS_COSTO); monto_aj = st.number_input("Monto (Negativo p/restar)", value=0.0); motivo = st.text_input("Motivo")
            if st.button("💾 APLICAR") and st.text_input("Clave Maestra", type="password", key="cl_aj") == CLAVE_MAESTRA:
                conn.execute("INSERT INTO ajustes_costos (centro_costo, monto, fecha, motivo) VALUES (?,?,?,?)", (cc_aj.upper(), monto_aj, hoy, motivo))
                conn.commit(); registrar_accion("AJUSTE", f"{cc_aj}: {monto_aj}"); guardar_en_drive(); st.rerun()
    conn.close()
...
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.53", layout="wide")
inicializar_db()
