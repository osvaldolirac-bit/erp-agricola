...
def guardar_en_drive():
    ...
    if drive:
        ...
        st.success("✅ Respaldo sincronizado en la nube.")
...
def modulo_dashboard():
    ...
    v_hoy = len(df_f[pd.to_datetime(df_f['fecha_vencimiento']).dt.date < hoy])
    with m3: 
        st.markdown(f"<div class='stMetric card-vencida'><small>⚠️ VENCIDAS</small><br><span class='metric-blue'>{v_hoy}</span><br><small style='color:#1976d2;'>DOCUMENTOS</small></div>", unsafe_allow_html=True)
    
    with m4: 
        st.markdown(f"<div class='stMetric'><small>📄 PENDIENTES</small><br><span class='metric-std'>{len(df_f)}</span><br><small>EN CARTERA</small></div>", unsafe_allow_html=True)
    with m5: 
        st.markdown(f"<div class='stMetric'><small>⛽ PETRÓLEO</small><br><span class='metric-std'>{f_decimal(saldo_pet)}L</span><br><small>SALDO NETO</small></div>", unsafe_allow_html=True)
    ...
    with c_izq:
        ...
        # SQL CORREGIDO: Elimina 'BODEGA' como Centro de Costo
        query = """SELECT UPPER(TRIM(cc)) as cc, SUM(val) as total FROM (
    ...
    with t2:
        with st.form("f_salida"):
            ...
            if st.form_submit_button("REGISTRAR SALIDA"):
                ...
                conn.commit(); registrar_accion("SALIDA PETROLEO", f"{l_s}L para {v}"); st.rerun()
    with t3:
        # PANTALLA: TODO COMPLETO
...
def modulo_bodega():
    ...
    with t1:
        ...
        n_nom, n_st = st.text_input("Nombre", item['producto']), st.number_input("Stock", value=float(item['stock']))
    ...
    with t2:
        ...
        sel_cc = []
        for cc_n in CENTROS_COSTO:
            if st.checkbox(cc_n, key=f"mb_{cc_n}"): sel_cc.append(cc_n)
    ...
    with t3:
        with st.form("nuevo_form"):
            n_p = st.text_input("Producto"); n_f = st.selectbox("Familia", FAMILIAS_PRODUCTOS)
            n_s, n_pn = st.number_input("Stock", 0.0), st.number_input("Precio", 0.0)
...
st.set_page_config(page_title="ERP LA CONCEPCIÓN v10.8.73", layout="wide")
inicializar_db()
