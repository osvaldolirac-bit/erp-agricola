...
CODIGO_DIRS_EXCLUIDOS = frozenset({"__pycache__", ".git", "snap", ".cache", ".venv", "node_modules"})
CODIGO_SUFIJOS_EXCLUIDOS = (".db", ".db.gz", ".pyc", ".log", ".RESPALDO_OFICIAL")

SPECS_RESPALDO_CODIGO = [
    {
        "nombre": "ERP Agrícola La Concepción",
        "slug": "concepcion",
        ...
        "roots": [
            ...
            {"path": "/etc/systemd/system", "files": ["erp-lc-web.service"]},
        ],
    },
    {
        "nombre": "ERP DEMO AGRICOLA",
        "slug": "demo",
        "db": "/root/demo/erp_demo.db",
        "secrets": "/root/demo/.streamlit/secrets.toml",
        "roots": [
            {"path": "/root/demo", "patterns": ["*.py"]},
            {"path": "/root/demo/.streamlit", "files": ["config.toml"]},
            {"path": "/root", "patterns": [
                "erp_*.py",
                "manual_contenido.py",
                "pppl_catalogo_sag.py",
                "demo_seed.py",
                "erp_ui_nav.py",
            ]},
            {"path": "/root/scripts", "recursive": True},
...
def obtener_config_respaldo(conn):
    return {
        ...
        "activo": meta_get(conn, META_ACTIVO, "1") == "1",
...
def enviar_respaldo_codigo_correo(nombre_erp, destinatarios, smtp_conf, archivo_tar, n_archivos, cuando=None):
    """Correo con adjunto. Gmail bloquea ZIP con código → se envía .zip.enc cifrado."""
    ...
    tamano_kb = os.path.getsize(archivo_tar) / 1024.0
    if tamano_kb > 22000:
        return False, f"Archivo demasiado grande para correo ({tamano_kb:.0f} KB)."
    nombre_zip = os.path.basename(archivo_tar)
    asunto = f"[{nombre_erp}] Respaldo de código {cuando.strftime('%d-%m-%Y')}"

    # Cifrar para el adjunto (Gmail rechaza ZIP/tar con fuentes .py).
    try:
    ...
    interior = f"""
            ...
            <p><b>Adjunto:</b> {nombre_enc} ({tamano_enc_kb:.1f} KB · {n_archivos} archivos en el ZIP interno)</p>
            <p><b>Copia ZIP en el VPS:</b> <code>{archivo_tar}</code></p>
            <p style="color:#555;line-height:1.5;">
                Gmail bloquea ZIP/tar con código fuente, por eso el adjunto va
...
def ejecutar_respaldo_codigo(conn, spec, secrets_path, forzar=False, usuario="SISTEMA"):
    ...
    try:
        ...
        ok, err = enviar_respaldo_codigo_correo(
            nombre_erp, destinatarios, smtp, tar_path, n_arch,
        )
        if not ok:
            meta_set(conn, META_CODIGO_ULTIMO_ERROR, err)
            conn.commit()
        ...
        meta_set(conn, META_CODIGO_ULTIMO_ERROR, "")
        conn.commit()
