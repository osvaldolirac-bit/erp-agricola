...
def register_ops_routes(app, login_required):
    ...
    # ── Centros de costo ──────────────────────────────────────────
    @app.route("/centros-costo/", methods=["GET", "POST"])
    @login_required
    def centros_costo():
        db = core.conn()
        ...
        if request.method == "POST":
            ...
            if action == "crear":
                ok, msg = ops_cc.crear_centro(db, request.form.get("nombre") or "")
                if ok:
                    db.commit()
                    flash(msg, "ok")
                else:
    ...
    def compras_form(fid: int | None = None):
        ...
        if request.method == "POST" and (row or oc):
            ...
            try:
                ...
                rubro_id = 0
            cc_ids_raw = request.form.getlist("centro_costo_id")
            try:
                cc_selected = [int(x) for x in cc_ids_raw if str(x).strip()]
            except ValueError:
                cc_selected = []
            ...
            if not proveedor_id or not documento:
                ...
                flash("Seleccione al menos un centro de costo", "danger")
                ...
                try:
                    if row:
                        db.execute(
                            ...
                            afecta_stock=?, notas=?, rubro_id=? WHERE id=?
                            """,
                            (
                                documento,
                                tipo_documento,
                        ...
                        cur = db.execute(
                            ...
                            (documento, tipo_documento, proveedor_id, concepto, fecha_emision, fecha_vencimiento,
                             neto, iva, total, pagado, saldo, estado, afecta_stock, notas, orden_compra_id, rubro_id)
                            VALUES (?,?,?,?,?,?,?,?,?,0,?,'pendiente',?,?,?,?)
                            """,
                            (
                                documento,
