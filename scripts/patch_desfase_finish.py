...
def ensure_fmt(path: Path) -> None:
    t = path.read_text(encoding="utf-8")
    if "def _fmt_cantidad_desfase" in t:
        print("fmt ok", path)
        return
    if "def _normalizar_um_lc_bodega" not in t:
        print("no normalizar in", path)
        return
    t = t.replace("def _normalizar_um_lc_bodega(um):", FMT + "def _normalizar_um_lc_bodega(um):", 1)
    path.write_text(t, encoding="utf-8")
    print("fmt inserted", path)
...
"""
    ...
    if not m:
        raise SystemExit("helpers missing in concepcion")
    NEW = m.group(1)
    if OLD_CANT not in t:
        raise SystemExit("OLD_CANT not in demo")
...
"""
    if OLD_FILTER not in t:
        raise SystemExit("OLD_FILTER not in demo")
...
"""
    if OLD_LC_SIN not in t:
        raise SystemExit("OLD_LC_SIN not in demo")
    ...
    path.write_text(t, encoding="utf-8")
    print("OK demo patched")


def main() -> int:
    ensure_fmt(Path("/root/demo-web/app_concepcion.py"))
    patch_demo()
    ensure_fmt(Path("/root/demo-web/app_demo.py"))
    return 0
