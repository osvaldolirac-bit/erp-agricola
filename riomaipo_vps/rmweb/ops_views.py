...
def register_ops_routes(app, login_required):
    ...
    def compras_form(fid: int | None = None):
        ...
        if row:
            ...
            directo = (request.args.get("directo") == "1") or (
                request.method == "POST" and request.form.get("compra_directa") == "1"
            )
            try:
        ...
        if request.method == "POST" and (row or oc or directo):
            try:
            ...
            if not row and orden_compra_id:
            ...
            if not proveedor_id or not documento:
                ...
                try:
                    ...
                    db.commit()
                    if row:
                        flash("Compra guardada", "ok")
                    elif orden_compra_id:
                        flash("Compra emitida desde OC", "ok")
                    else:
                        flash("Compra directa registrada", "ok")
                    db.close()
    ...
    def bodega_list():
        ...
        rows_raw = db.execute(
            """
            SELECT i.*, p.codigo AS prod_codigo, p.id AS pid
            FROM inventario i
        ...
        from rmweb import ops_arriendo
        ops_arriendo.ensure_arriendo_schema(db)
        en_map = ops_arriendo.stock_en_arriendo_map(db)
        rows = []
        for r in rows_raw:
            d = dict(r)
            pid = int(r["pid"]) if r["pid"] else None
            stock = float(r["stock"] or 0)
            en_a = en_map.get(pid, 0.0) if pid else 0.0
            d["en_arriendo"] = en_a
            d["disponible"] = max(0.0, stock - en_a)
            rows.append(d)
        movs = db.execute(
            """
        ...
        productos = db.execute(
            ...
            WHERE activo=1 AND COALESCE(maneja_stock,0)=1
            ORDER BY nombre
            """
        ).fetchall()
        catalogo = db.execute(
            ...
            SELECT id, codigo, nombre, unidad, precio,
                   ...
                   COALESCE(tipo_item,'venta') AS tipo_item,
                   COALESCE(tarifa_arriendo,0) AS tarifa_arriendo,
    ...
    def bodega_producto_form(pid: int | None = None):
        ...
        if request.method == "POST":
            ...
            tipo_item = (request.form.get("tipo_item") or "venta").strip().lower()
            if tipo_item not in {"venta", "servicio", "arriendo"}:
                tipo_item = "venta"
            if tipo_item == "arriendo":
                es_servicio = 1
                maneja_stock = 1
            ...
            try:
                tarifa_arriendo = float(request.form.get("tarifa_arriendo") or 0)
            except ValueError:
                tarifa_arriendo = 0.0
            unidad_arriendo = (request.form.get("unidad_arriendo") or "día").strip() or "día"
            categoria = (request.form.get("categoria") or "").strip() or None
            activo = 1 if request.form.get("activo") == "1" else 0
            if tipo_item == "arriendo" and not es_servicio:
                es_servicio = 1
            if not nombre:
                    ...
                    if edit:
                        db.execute(
                            ...
                            UPDATE productos SET codigo=?, nombre=?, unidad=?, precio=?,
                              ...
                              tipo_item=?, tarifa_arriendo=?, unidad_arriendo=?, categoria=?
                            WHERE id=?
                            """,
                            (
                                codigo, nombre, unidad, precio, es_servicio, maneja_stock, activo,
                                tipo_item, tarifa_arriendo, unidad_arriendo, categoria, edit["id"],
                            ),
                        )
    ...
    def arriendos_list():
        ...
        rows = db.execute(
            """
            SELECT ac.*, cl.razon_social AS cliente, cot.folio AS cot_folio
            FROM arriendo_contratos ac
            LEFT JOIN clientes cl ON cl.id=ac.cliente_id
            LEFT JOIN cotizaciones cot ON cot.id=ac.cotizacion_id
            ORDER BY ac.id DESC
            """
        ).fetchall()
        db.close()
        return render_template("arriendos/lista.html", active="arriendos", rows=rows)

    @app.route("/arriendos/<int:aid>")
    @login_required
    def arriendos_detalle(aid: int):
        from rmweb import ops_arriendo
        ...
        if not row:
            ...
            return redirect(url_for("arriendos_list"))
        items = db.execute(
            "SELECT * FROM arriendo_items WHERE contrato_id=? ORDER BY id",
            (aid,),
        ).fetchall()
        en_map = ops_arriendo.stock_en_arriendo_map(db)
        stock_rows = []
        for it in items:
            ...
            if it["producto_id"]:
                ...
                stock = float(inv["stock"] or 0) if inv else 0.0
                d["stock_disponible"] = ops_arriendo.stock_disponible(
                    db, int(it["producto_id"]), stock
                )
            else:
                d["stock_disponible"] = None
            stock_rows.append(d)
        db.close()
        return render_template(
            "arriendos/detalle.html",
            active="arriendos",
            row=row,
            items=stock_rows,
            hoy=core.hoy_chile().isoformat(),
    ...
    @app.route("/arriendos/item/<int:iid>/entrega", methods=["POST"])
    @login_required
    def arriendos_entrega(iid: int):
        from rmweb import ops_arriendo
        ...
        try:
            ...
            cant = 0
        fecha = (request.form.get("fecha") or core.hoy_chile().isoformat()).strip()
        ok, msg = ops_arriendo.registrar_entrega(db, iid, cant, fecha)
        aid = db.execute(
            "SELECT contrato_id FROM arriendo_items WHERE id=?", (iid,)
        ).fetchone()
        if ok:
            db.commit()
            flash(msg, "ok")
            ...
            flash(msg, "danger")
        db.close()
        return redirect(url_for("arriendos_detalle", aid=int(aid["contrato_id"])) if aid else url_for("arriendos_list"))

    @app.route("/arriendos/item/<int:iid>/devolucion", methods=["POST"])
    @login_required
    def arriendos_devolucion(iid: int):
        from rmweb import ops_arriendo
        ...
        try:
            ...
            cant = 0
        fecha = (request.form.get("fecha") or core.hoy_chile().isoformat()).strip()
        ok, msg = ops_arriendo.registrar_devolucion(db, iid, cant, fecha)
        aid = db.execute(
            "SELECT contrato_id FROM arriendo_items WHERE id=?", (iid,)
        ).fetchone()
        if ok:
