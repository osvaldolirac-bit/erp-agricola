...
(function () {
  ...
  function sanitizarDecimalComa(raw) {
  ...
  function initDecimalComaInputs() {
    document.querySelectorAll('input.js-decimal-coma').forEach(function (input) {
      ...
      input.dataset.decimalComaBound = '1';
      input.setAttribute('inputmode', 'decimal');
      input.setAttribute('lang', 'es-CL');
      input.addEventListener('input', function () {
        var next = sanitizarDecimalComa(input.value);
      ...
      input.addEventListener('keypress', function (e) {
        ...
        if (/\d/.test(ch)) return;
        if (ch === ',' && input.value.indexOf(',') === -1) return;
        e.preventDefault();
      });
      input.addEventListener('paste', function (e) {
        e.preventDefault();
        var text = (e.clipboardData || window.clipboardData).getData('text');
        input.value = sanitizarDecimalComa(text);
      });
    });
  }
  ...
  window.demoInitDecimalComa = initDecimalComaInputs;
})();
