...
def enviar_alerta(registro: dict[str, Any]) -> bool:
    ...
    interior = f"""
        <p style="color:#1F2933;line-height:1.55;margin:0 0 12px;">
          Se registró una salida de petróleo desde <b>Salida QR</b> (informativa).
          Un administrador debe <b>autorizarla</b> en Petróleo → Salida QR para imputarla al estanque.
        </p>
...
def enviar_alerta_autorizacion(registro: dict[str, Any], usuario: str) -> bool:
    ...
    interior = f"""
        <p style="color:#1F2933;line-height:1.55;margin:0 0 12px;">
          Se <b>autorizó</b> la salida QR <b>{html_esc(codigo)}</b> y quedó
...
def autorizar_salida(codigo: str, usuario: str) -> dict[str, Any]:
    ...
    try:
        ...
        if litros <= 0:
            return {"ok": False, "msg": "Litros inválidos en la Salida QR."}

        cuarteles = _cuarteles_desde_huerto(huerto)
        if not cuarteles:
            return {"ok": False, "msg": "No se pudieron resolver los cuarteles de la Salida QR."}
...
def rechazar_salida(codigo: str, usuario: str, motivo: str = "") -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "msg": f"{codigo} rechazado. El código se conserva en Salida QR (sin imputar al estanque).",
        "codigo": codigo,
    }
