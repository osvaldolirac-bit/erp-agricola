...
RUBRO_PETROLEO_MATRIZ = "Petróleo"
...
def cuarteles_costos_lc(cuarteles: list[str] | None) -> list[str]:
    """Quita cuartel EL ESPINO de Flujo/Dashboard en tenant LC."""
...
def cuarteles_vista_costos_lc(cuarteles: list[str] | None) -> list[str]:
    """Pestañas del módulo Costos LC: incluye EL ESPINO (solo petróleo imputado)."""
    return list(cuarteles or [])


def ocultar_cuartel_espino_en_matriz_lc(matriz: pd.DataFrame | None) -> pd.DataFrame | None:
    """Anula gastos migrados de EL ESPINO; conserva salidas de petróleo (solo LC)."""
    ...
    mask = body["Rubro"] != RUBRO_PETROLEO_MATRIZ
    body.loc[mask, CUARTEL_ESPINO_LC] = 0.0
