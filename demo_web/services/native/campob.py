...
def _redirect_campob(sec: str, temp: str, **extra) -> redirect_module:
    return redirect_module("campob", sec=sec, temp=temp, **extra)
...
def _historial(demo, conn, nombre: str, fi, ff) -> dict:
    ...
    if not df_base.empty:
        ...
        if not df_show.empty:
            ...
            if blob:
                pdf_url = url_for("modules.pdf_download", token=store_pdf(blob, f"campob_{nombre}.pdf"))
...
def gather_campob(user_email: str, user_rol: str) -> dict:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if fn:
                ...
                return _redirect_campob(**extra)
        finally:
            conn.close()

    ctx = gather_campob(user_email, user_rol)
