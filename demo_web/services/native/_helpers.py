...
def parse_decimal_cl(raw: str | None, default: float | None = 0.0) -> float | None:
    """Parsea cantidad/monto: formato chileno (1,5 / 1.250) o HTML number (220.0).

    Acepta coma decimal chilena, miles con punto (1.250) y valores de
    ``<input type="number">`` que envían punto decimal (220.0, 5419.5).
    """
    import re
    ...
    if re.fullmatch(r"\d+(,\d+)?|\d{1,3}(\.\d{3})+(,\d+)?", s):
        ...
        try:
            ...
            return default
    # Punto decimal típico de inputs HTML (220.0, 5419.5) vs miles CL (1.250).
