...
# Especies de cultivo en Libro de Campo (≠ GAP_ESPECIES del módulo GlobalGAP / ámbito consultor).
_LIBRO_CAMPO_ESPECIES_DEFAULT = ("Cerezos", "Ciruelos", "Nogales")


def _especies_libro_campo(demo) -> list[str]:
    """Lista de especies para ingreso LC; no usar GAP_ESPECIES (razones sociales / ámbitos)."""
...
def _ingreso(demo, conn) -> dict:
    ...
    siguiente = int(res) + 1 if res else 1
...
def _historial(demo, conn) -> dict:
    ...
    if q_prod:
        filtros.append("producto LIKE ?")
        params.append(f"%{q_prod}%")
    if q_app.isdigit():
...
def _post_guardar_evento(demo, conn) -> dict:
    ...
    res = conn.execute("SELECT MAX(n_aplicacion) FROM libro_campo").fetchone()[0]
    n_app = int(res) + 1 if res else 1

    for item in car:
        demo._insertar_linea_libro_campo(
            conn, fe_app, n_app, huerto, especie, item, total_agua, aplicador, op_cert, maquinaria, tractor
        )
    conn.commit()
    prods_txt = ", ".join(i["producto"] for i in car)
    demo.registrar_accion("LIBRO CAMPO", f"App N°{n_app} - {prods_txt}")
