...
def _puede_editar_admin(demo) -> bool:
    """Operador/certificación pueden ver Administración; solo admin edita."""
    try:
...
def _gather_flujo(demo, conn) -> dict:
    from erp_flujo_financiero import (
        cargar_egresos_teso_mes,
        cargar_ingresos_cc,
        cargar_notas_egresos_teso_mes,
    ...
    eg_teso = cargar_egresos_teso_mes(conn, temp_nombre)
    notas_eg = cargar_notas_egresos_teso_mes(conn, temp_nombre)
    caja_ini = cargar_saldo_caja_inicial(conn, temp_nombre)
    es_vigente = fi <= hoy_demo(demo) <= ff
    ...
    total_eg = 0.0
    for anio, mes in meses:
        monto = float(eg_teso.get((anio, mes), 0.0) or 0.0)
        total_eg += monto
        egresos_meses.append(
            {
                "anio": anio,
                "mes": mes,
                "label": _mes_label(anio, mes),
                "monto": monto,
                "nota": notas_eg.get((anio, mes), ""),
            }
        )
    ...
    return {
        ...
        "egresos_flujo_meses": egresos_meses,
        "egresos_flujo_total": float(total_eg),
        "egresos_flujo_total_fmt": demo.f_peso(total_eg),
    }
...
def _post_mail_teso(demo, conn) -> dict:
    ...
    if tiene_mail_pet:
        msg = "Mail Tesorería y Salida Link petróleo actualizados."
    return {"ok": True, "msg": msg}
...
def _post_guardar_egresos_flujo(demo, conn) -> dict:
    from erp_flujo_financiero import guardar_egresos_teso_mes, iter_meses_rango
    ...
    notas = {}
    for anio, mes in meses:
        key = f"eg_{anio}_{mes}"
        nota_key = f"eg_nota_{anio}_{mes}"
        try:
            monto = float(request.form.get(key) or 0)
        except ValueError:
            return {"ok": False, "msg": f"Monto inválido en {_mes_label_admin(anio, mes)}."}
        if monto < 0:
            return {"ok": False, "msg": "Los egresos proyectados no pueden ser negativos."}
        datos[(anio, mes)] = monto
        notas[(anio, mes)] = (request.form.get(nota_key) or "").strip()
    guardar_egresos_teso_mes(conn, temp, datos, notas)
    demo.registrar_accion("FLUJO EGRESOS TESO", temp)
    return {"ok": True, "msg": f"Egresos de tesorería proyectados guardados para {temp}."}


def _mes_label_admin(anio, mes):
    nombres = (
...
def view(user_email: str, user_rol: str):
    ...
    if request.method == "GET" and request.args.get("action") == "download_db":
        ...
        return redirect_module("admin")

    if request.method == "POST":
        action = request.form.get("action", "")
        if action in ACCIONES_MOVIDAS_A_MASTER:
            flash(MSG_MOVIDO_A_MASTER, "warning")
        ...
        try:
            ...
            elif action == "guardar_egresos_flujo":
                result = _post_guardar_egresos_flujo(demo, conn)
            elif action == "reseed":
            ...
            if result:
                ...
                extra = {"sec": sec}
                if action in ("guardar_metas", "guardar_ingresos_flujo", "guardar_egresos_flujo"):
