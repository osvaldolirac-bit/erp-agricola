...
def migrar_personal_salida_petroleo(conn) -> None:
    """Columna + extras (dueños) para el formulario Salida Link de petróleo."""
...
def _remuneraciones(demo, conn) -> dict:
    ...
    primera_def = demo._primera_cuota_date_default(ficha).isoformat() if ficha else hoy_demo(demo).isoformat()
    return {
        "meses": MESES,
        "mes_sel": prov_m,
        "anio_sel": prov_a,
...
def _post_crear_auth_extra(demo, conn) -> dict:
    ...
    if existe:
        ...
        return {"ok": True, "msg": f"{nom} ya existía; quedó activo para Salida Link."}
    conn.execute(
...
def _post_registrar_prestamo(demo, conn) -> dict:
    ...
    primera = request.form.get("primera_cuota") or str(hoy_demo(demo))
    pc_m, pc_a = demo._mes_anio_desde_fecha(parse_date(primera, hoy_demo(demo)))
    ficha = conn.execute(
...
def view(user_email: str, user_rol: str):
    ...
    if request.method == "POST":
        if demo.es_solo_lectura():
            ...
            return _redirect_rrhh(sec=request.form.get("sec") or request.args.get("sec", "personal"))
