...
def _post_corregir_stock(demo, conn) -> dict:
    ...
    try:
        iid = int(request.form.get("producto_id") or 0)
        nst = float(request.form.get("stock") or 0)
        npmp = float(request.form.get("precio_medio") or 0)
    except (TypeError, ValueError):
...
def _post_corregir_mov(demo, conn) -> dict:
    ...
    try:
        ...
        nc = float(request.form.get("cantidad") or 0)
        nv = float(request.form.get("valor") or 0)
    except (TypeError, ValueError):
        return {"ok": False, "msg": "Datos inválidos."}
    ...
    if delta_stock < 0 and stock_inv + delta_stock < -1e-9:
        return {"ok": False, "msg": f"Stock insuficiente para aumentar la salida (disponible: {demo.f_cantidad(stock_inv)} {um_inv})."}
    conn.execute(
