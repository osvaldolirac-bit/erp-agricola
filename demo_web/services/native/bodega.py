...
def _post_corregir_stock(demo, conn) -> dict:
    ...
    conn.execute(
        """UPDATE inventario SET producto=?, familia=?, unidad_medida=?,
           precio_medio=?, ingrediente_activo=? WHERE id=?""",
        (nprod, nfam, num_um, npmp, nia, iid),
    )
    if _is_espino_tenant():
        from demo_web.services.native import espino_bodega

        nst = espino_bodega.reconciliar_stock_kardex_espino(
            demo, conn, iid, float(nst), pmp=float(npmp), um=num_um,
        )
    else:
        conn.execute("UPDATE inventario SET stock=? WHERE id=?", (nst, iid))
