...
from demo_web.services.espino_scope import cuarteles_espino, prorrateo_pct_espino
...
def _restore_lc_defaults(erp: Any) -> None:
    ...
    if "RAZONES_SOCIALES_COMPRAS" in _LC_DEFAULTS:
...
def _apply_espino_tenant_overrides(erp: Any) -> None:
    """Tenant El Espino: costos por variedad (prorrateo ha)."""
    variedades = cuarteles_espino()
    prorr = prorrateo_pct_espino()
    ...
    erp.PRORRATEO_CC_DEFAULT = dict(prorr)
    erp.GAP_ESPECIES = ["Cerezos"]
    erp.GAP_ESPECIE_CUARTELES = {"Cerezos": list(variedades)}
