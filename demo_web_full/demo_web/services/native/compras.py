...
def _siguiente_correlativo_interno(conn) -> str:
    """Siguiente correlativo solo entre documentos con N° de factura real (no INT-)."""
    row = conn.execute(
        """
...
def _historial(demo, conn) -> dict:
    ...
    siguiente_corr = _siguiente_correlativo_interno(conn) if puede_folio else ""
...
def _post_asignar_folio_interno(demo, conn) -> dict:
    ...
    dup = conn.execute(
        ...
        WHERE TRIM(COALESCE(folio_interno,''))=? AND id!=?
          AND nro_documento NOT LIKE '%_P'
          AND UPPER(TRIM(nro_documento)) NOT LIKE 'INT-%'
          AND UPPER(TRIM(nro_documento)) NOT LIKE 'INT/%'
        """,
    ...
    if dup:
        return {"ok": False, "msg": f"El correlativo {folio_new} ya está usado en otra factura."}

    conn.execute("UPDATE facturas SET folio_interno=? WHERE id=?", (folio_new, fid))
    conn.commit()
    demo.registrar_accion(
        "COMPRA",
        f"Correlativo ID {fid} (factura {doc}): {folio_old or '—'} → {folio_new}",
    )
    return {
        "ok": True,
...
def _post_corregir_factura(demo, conn) -> dict:
    ...
    _ensure_folio_interno_col(conn)
    folio_new = (request.form.get("folio_interno") or "").strip()
    # Correlativo solo si el N° documento es factura real
