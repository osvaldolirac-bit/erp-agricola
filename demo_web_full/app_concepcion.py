...
class _CursorSoloLectura:
    ...
    def execute(self, sql, parameters=(), /, *args, **kwargs):
        if self._bloquear and _sql_es_modificacion(sql):
...
def encabezado_pdf(pdf, titulo, modo_petroleo=False, saldo_petroleo=None):
    ...
    if not logo:
        ...
        pdf.set_xy(10, 10)
        pdf.cell(80, 6, "AGRICOLA LA CONCEPCION")
...
def generar_pdf_libro_campo(df):
    try:
        ...
        pdf.set_auto_page_break(auto=True, margin=14)
        pdf.add_page()
        encabezado_pdf(pdf, "REGISTRO FITOSANITARIO")
...
def _imputar_costos_rrhh(conn, trabajador_id, mes, anio, total):
    ...
    try:
        ...
        pass
    for cc_interno in CUARTELES_PRORRATEO:
        parte_costo = total * prorrateo_rrhh.get(cc_interno, 0.0)
        if parte_costo > 0:
            try:
                conn.execute(
...
def modulo_tesoreria():
    ...
    def enviar_correo_pago_interno(proveedor, documentos, monto_total, metodo, usuario_operador):
        ...
        try:
            ...
            msg['From'] = smtp_from_header(emisor)
            msg['To'] = ", ".join(DESTINATARIOS)
            msg['Subject'] = f"🚨 [{NOMBRE_ERP}] Pago Procesado: {proveedor} ({n_docs} doc.)"
...
def modulo_seguridad():
    ...
    if sec == adm_secciones[0]:
        ...
        st.markdown("#### Prorrateo de centros de costo (fundo)")
        st.caption(
