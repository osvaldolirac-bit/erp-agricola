...
from erp_demo_alertas import (
    dias_restantes_prueba,
    migrar_alertas_vencimiento_demo,
    migrar_ampliacion_prueba_30d,
)
...
# =============================================================================
NOMBRE_DB = '/root/demo/erp_demo.db'
SECRETS_PATH = "/root/demo/.streamlit/secrets.toml"
NOMBRE_ERP = "ERP DEMO AGRICOLA"
DEMO_URL = "https://erpmaster.cl/agricola/login"
DEMO_DIAS_PRUEBA = 30
USUARIOS_DEMO_PERMANENTES = {
...
PERFILES_USUARIO_TXT = {
    ...
    "admin_cliente": "Administrador de campo",
    "admin": "Administrador de campo",
    "operador": "Operación",
    "certificacion": "Certificación GlobalGAP",
}
CLAVE_MAESTRA = "2908" 
IMPUESTO_ESPECIFICO_LITRO = 75 
CORREOS_TESORERIA_DEFAULT = ("osvaldolirac@gmail.com",)
DATA_ESP_HISTORICA = []
...
def _ruta_logo_erpmaster():
    base = os.path.dirname(os.path.abspath(__file__))
    for ruta in (
        os.path.join(LOGO_DIR, "logo_erpmaster.png"),
        os.path.join(base, "static", "logo_erpmaster.png"),
    ):
        if os.path.isfile(ruta):
            return ruta
    return None

def inyectar_sello_erpmaster_demo():
    """Sello fijo ERPMASTER: inyecta en document.body vía iframe (Streamlit ignora width en img dentro de markdown)."""
    uri = _archivo_a_data_uri(_ruta_logo_erpmaster())
    if not uri:
        return
    logo_w = 120
    components.html(
        ...
        (function () {{
            ...
            var id = "erpmaster-sello-root";
            var prev = doc.getElementById(id);
            if (prev) prev.remove();
            var root = doc.createElement("div");
            root.id = id;
            root.setAttribute("data-v", "5");
            root.style.cssText =
                "position:fixed;right:12px;bottom:10px;z-index:999999;" +
            ...
            img.alt = "ERPMASTER";
            img.style.cssText =
                "width:{logo_w}px;height:auto;display:block;max-width:none !important;" +
                "min-width:{logo_w}px;object-fit:contain;";
            var txt = doc.createElement("p");
            txt.style.cssText =
                "margin:0;font-size:11px;font-weight:600;letter-spacing:0.03em;" +
                "text-transform:uppercase;color:rgba(27,58,87,0.75);white-space:nowrap;";
            txt.innerHTML = "Desarrollado por <strong style=\\"font-weight:800;\\">ERPMASTER</strong>";
            root.appendChild(img);
            root.appendChild(txt);
            doc.body.appendChild(root);
            var mq = window.parent.matchMedia("(max-width:768px)");
            var applyMobile = function () {{
                img.style.width = mq.matches ? "96px" : "{logo_w}px";
                img.style.minWidth = mq.matches ? "96px" : "{logo_w}px";
            }};
            applyMobile();
...
def _ruta_bg_dashboard_demo():
    candidatos = (
        "/root/static/bg_dashboard_demo.jpg",
        "/root/static/bg_dashboard_demo.png",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "bg_dashboard_demo.jpg"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "bg_dashboard_demo.png"),
    )
...
def inyectar_fondo_dashboard():
    uri = _archivo_a_data_uri(_ruta_bg_dashboard_demo())
    if not uri:
        return
    st.markdown(
        f"""
...
def _ruta_bg_maquinaria():
    candidatos = (
        ...
        "/root/demo/static/bg_maquinaria.jpg",
        "/root/demo/static/bg_maquinaria.png",
...
def inyectar_css_login_pantalla():
    ...
    st.markdown(
        ...
        .login-hero-fixed .login-hero-icon {{
            display: block;
            font-size: clamp(3.2rem, 9vw, 4.75rem) !important;
        ...
        [data-testid="stAppViewContainer"]:has(.login-bg-marker) [class*="st-key-login_toggle"] button[data-testid="stBaseButton-primary"] {{
            width: auto !important; min-width: 12.5rem !important;
            padding: 1rem 2.35rem !important; min-height: 3.85rem !important;
            border-radius: 999px !important;
            background: linear-gradient(135deg, #FF9800, #E65100) !important;
            background-color: #E65100 !important;
            border: 2px solid rgba(255, 255, 255, 0.65) !important;
            box-shadow: 0 12px 36px rgba(230, 81, 0, 0.55), 0 4px 16px rgba(0, 0, 0, 0.35) !important;
            color: #ffffff !important;
        }}
        ...
        [data-testid="stAppViewContainer"]:has(.login-bg-marker) [class*="st-key-login_toggle"] button[kind="primary"]:hover {{
            ...
            color: #ffffff !important;
            box-shadow: 0 14px 40px rgba(230, 81, 0, 0.62), 0 4px 18px rgba(0, 0, 0, 0.38) !important;
        }}
...
def _login_fondo_html(hero_inner_html):
    return f"""
...
TEMAS_MODULO = {
    ...
    "Campob": {"color": "#558B2F", "claro": "#F1F8E9", "sub": "Registro y seguimiento de gastos Campo B"},
    ...
    "Manual": {"color": "#455A64", "claro": "#ECEFF1", "sub": "Guía de uso y documentación del sistema demo"},
    "GlobalGAP": {"color": "#00695C", "claro": "#E0F2F1", "sub": "Certificación, trazabilidad y cumplimiento IFA (demo)"},
}
...
CENTROS_COSTO = ["CUARTEL A", "CUARTEL B", "CUARTEL C", "CUARTEL D", "CUARTEL E", "CAMPO B", "CUARTEL F"]
METODOS_PAGO_TESORERIA = ["Transferencia", "Efectivo", "Cheque", "Tarjeta bancaria"]
CUARTELES_OFICIALES = CENTROS_COSTO
RAZONES_SOCIALES_COMPRAS = ["ERP Demo Agrícola", "La Concepción", "El Espino"]
...
ETIQUETAS_MAQ = ["Conforme", "Cerrado", "Pendiente", "En Observación", "Crítico"]

ROLES_USUARIO = ("super_admin", "admin_cliente", "admin", "operador", "certificacion")
SUPER_ADMIN_EMAILS = frozenset({"osvaldolira@laconcepcion.cl"})
PERFILES_ASIGNABLES_CLIENTE = ("operador", "certificacion", "admin_cliente")
PERFILES_ASIGNABLES_SUPER = ("operador", "certificacion", "admin_cliente", "super_admin")
MENU_COMPLETO = [
    ("🏠 DASHBOARD", "DASHBOARD"),
    ("📦 COMPRAS", "Compras"),
    ("💸 TESORERÍA", "Tesoreria"),
...
GAP_ESPECIES = ["ESPECIE 1", "ESPECIE 2"]
GAP_ESPECIE_CUARTELES = {
    "ESPECIE 1": ["CUARTEL A", "CUARTEL B"],
    "ESPECIE 2": ["CUARTEL C"],
}
...
def normalizar_rol_usuario(rol, email=None):
    """Unifica roles legacy y asegura super_admin solo en cuentas de plataforma."""
    rol_txt = str(rol or "operador").strip().lower()
    email_txt = str(email or "").strip().lower()
    if rol_txt == "admin":
        return "super_admin" if email_txt in SUPER_ADMIN_EMAILS else "admin_cliente"
    if rol_txt == "super_admin":
        if email_txt and email_txt not in SUPER_ADMIN_EMAILS:
            return "admin_cliente"
        return "super_admin"
    if rol_txt in ("admin_cliente", "operador", "certificacion"):
        return rol_txt
    return "operador"

def _rol_sesion():
    return normalizar_rol_usuario(
        st.session_state.get("rol"),
        st.session_state.get("email"),
    )

def es_super_admin():
    return _rol_sesion() == "super_admin"

def es_admin_cliente():
    return _rol_sesion() == "admin_cliente"
...
def perfiles_asignables_demo():
    return PERFILES_ASIGNABLES_SUPER if es_super_admin() else PERFILES_ASIGNABLES_CLIENTE

def etiqueta_perfil_demo(rol, email=None):
    email_use = email if email is not None else st.session_state.get("email")
    key = normalizar_rol_usuario(rol, email_use)
    return PERFILES_USUARIO_TXT.get(key, str(key or ""))

def _rol_bd_usuario(conn, email):
    row = conn.execute(
        "SELECT COALESCE(rol, 'operador') FROM usuarios WHERE email=?",
        (str(email or "").strip(),),
    ).fetchone()
    if not row:
        return "operador"
    return normalizar_rol_usuario(row[0], email)

def usuario_gestionable_demo(conn, email_objetivo):
    """admin_cliente no puede editar ni eliminar super_admin."""
    if es_super_admin():
        return True
    return _rol_bd_usuario(conn, email_objetivo) != "super_admin"

def contar_roles_admin_demo(conn):
    rows = conn.execute("SELECT email, COALESCE(rol, 'operador') FROM usuarios").fetchall()
    n_super = n_cliente = 0
    for em, rol in rows:
        r = normalizar_rol_usuario(rol, em)
        if r == "super_admin":
            n_super += 1
        elif r == "admin_cliente":
            n_cliente += 1
    return n_super, n_cliente
...
def construir_menu_usuario(email, rol, conn=None):
    ...
    if conn is None:
        ...
        cerrar = True
    rol_norm = normalizar_rol_usuario(rol, email)
    if rol_norm != "operador":
        opts = construir_menu_rol(rol_norm, email=email)
    else:
        row = conn.execute("SELECT modulos FROM usuarios WHERE email=?", (email,)).fetchone()
        modulos_txt = row[0] if row else None
        opts = construir_menu_rol(rol_norm, modulos_txt, email=email)
    if cerrar:
        conn.close()
    return opts
...
def es_solo_lectura():
    """Demo sin perfil lector; exportación PDF siempre habilitada."""
    return False

def puede_gestionar_pppl():
    return _rol_sesion() in ("super_admin", "admin_cliente", "certificacion")
...
CUARTELES_PRORRATEO = [c for c in CENTROS_COSTO if c != "CAMPO B"]
PESOS_PRORRATEO_RRHH = {
    "CUARTEL A": 0.0794,
    "CUARTEL B": 0.0794,
    "CUARTEL C": 0.3271,
    "CUARTEL D": 0.3271,
...
def _normalizar_prorrateo(pesos, cuarteles):
    sub = {k: pesos[k] for k in cuarteles if k in pesos and pesos[k] > 0}
    total = sum(sub.values())
    if total <= 0:
        n = len(cuarteles)
        return {k: 1 / n for k in cuarteles} if n else {}
    return {k: v / total for k, v in sub.items()}

PRORRATEO_RRHH = _normalizar_prorrateo(PESOS_PRORRATEO_RRHH, CUARTELES_PRORRATEO)
PRORRATEO_FUNDO = _normalizar_prorrateo(PESOS_PRORRATEO_FUNDO, CUARTELES_PRORRATEO)
...
MAPEO_CUARTELES_LEGACY = {
    "CEREZOS CORTE 1": "CUARTEL A",
    "CEREZOS CORTE 2": "CUARTEL B",
    "CIRUELOS": "CUARTEL C",
    "NOGALES APARICION": "CUARTEL D",
    "NOGALES CRUZ DEL SUR": "CUARTEL E",
    "OTROS": "CUARTEL F",
}
CLIMATE_DEMO_ESTACION = {
    "id": 3539,
    "codigo": 3539,
    "nombre": "Huelquén",
...
def _admin_form_ingresos_temporada(conn, temporada, fi, ff, key_prefix):
    ...
    st.caption(
        ...
        f"**{len(meses)} meses** · ingreso del fundo = suma de todos los CC."
    )
...
def _admin_tab_ingresos_flujo(conn):
    ...
    st.caption(
        ...
        "El ingreso total del fundo en cada mes es la **suma** de los CC."
    )
...
def conectar_db():
    conn = sqlite3.connect(NOMBRE_DB, timeout=30, check_same_thread=False)
    ...
    try:
        migrar_alertas_vencimiento_demo(conn)
        migrar_ampliacion_prueba_30d(conn, USUARIOS_DEMO_PERMANENTES)
    except Exception:
        pass
...
def registrar_accion(accion, detalle):
    ...
    try:
        ...
        conn.close()
        try:
            clear_fn = getattr(getattr(st, "cache_data", None), "clear", None)
...
def obtener_destinatarios_tesoreria(conn):
    try:
        ...
        dest = []
    vistos = {d.lower() for d in dest}
    for em in CORREOS_TESORERIA_DEFAULT:
        em = str(em).strip()
        if em and em.lower() not in vistos:
            dest.append(em)
            vistos.add(em.lower())
    return dest
...
def _form_mail_tesoreria_usuarios(conn, incluir_vigencia=False):
    ...
    st.caption(
        ...
        "(requiere SMTP en secrets)."
    )
    if incluir_vigencia:
        rows = conn.execute(
            """SELECT email, rol, COALESCE(mail_tesoreria, 0), fecha_expira
               FROM usuarios ORDER BY email"""
        ).fetchall()
        hc = st.columns([2.8, 1.1, 1.3, 1.2])
        ...
        rows = conn.execute(
            "SELECT email, rol, COALESCE(mail_tesoreria, 0) FROM usuarios ORDER BY email"
        ).fetchall()
        hc = st.columns([3.2, 1.4, 1.4])
        hc[0].markdown("**Usuario**")
        hc[1].markdown("**Perfil**")
    ...
    with st.form("seg_mail_teso"):
        for row in rows:
            if incluir_vigencia:
                email, rol, flag, fexp = row
                vig = "Permanente" if not fexp else str(fexp)
                c1, c2, c3, c4 = st.columns([2.8, 1.1, 1.3, 1.2])
                c1.caption(email)
                c2.caption(etiqueta_perfil_demo(rol))
                c3.caption(vig)
                with c4:
                ...
                with c3:
                    st.checkbox("Sí", value=bool(flag), key=f"mail_teso_{email}")
        if st.form_submit_button("💾 GUARDAR MAIL TESORERÍA"):
            for row in rows:
                email = row[0]
            ...
            conn.commit()
            registrar_accion("MAIL TESO USUARIOS", "Preferencias actualizadas")
            st.success("Mail respaldo Tesorería actualizado.")
            st.rerun()
    return [r[0] for r in rows]
...
def enviar_correo_alerta(usuario_intruso, exitoso=True):
    ...
    conf = _conf_smtp_demo()
    if not conf:
        return
    try:
        emisor = conf["emisor"]
        clave = conf["clave"]
        receptor = conf["receptor_admin"]
        ...
        if exitoso:
            msg['Subject'] = f"🚨 ALERTA: Acceso Detectado en ERP DEMO AGRICOLA"
            ...
            msg['Subject'] = f"🔥 ADVERTENCIA: Intento de Acceso RECHAZADO en ERP DEMO AGRICOLA"
        ...
        cuerpo = plantilla_correo_html(
            ...
            interior,
            nombre_erp="ERP DEMO AGRICOLA",
            pie="Correo automático de seguridad — ERP DEMO AGRICOLA.",
        )
...
def _leer_secrets_toml_archivo(ruta):
    if not os.path.isfile(ruta):
        return None
    try:
        from erp_respaldo import _cargar_toml

        return _cargar_toml(ruta)
    except Exception:
        return None

def _rutas_secrets_demo():
    base = os.path.dirname(os.path.abspath(__file__))
    rutas = []
    sp = globals().get("SECRETS_PATH")
    if sp:
        rutas.append(sp)
    rutas.extend([
        os.path.join(base, ".streamlit", "secrets.toml"),
        "/root/demo/.streamlit/secrets.toml",
...
def _bloque_gmail_smtp():
    try:
        if "gmail_smtp" in st.secrets:
            return dict(st.secrets["gmail_smtp"])
    except Exception:
        pass
    for ruta in _rutas_secrets_demo():
        data = _leer_secrets_toml_archivo(ruta)
        if data and "gmail_smtp" in data:
            return data["gmail_smtp"]
    return None

def _conf_smtp_demo():
    try:
        conf = _bloque_gmail_smtp()
        if not conf:
            return None
...
def _normalizar_lista_correos(*valores):
    vistos = set()
    salida = []
    for val in valores:
        if not val:
...
def _destinatarios_admin_demo(admin_email=None):
    """Copias admin solo al correo configurado en secrets (correo_receptor)."""
    conf = _conf_smtp_demo()
    receptor = (conf or {}).get("receptor_admin")
    return _normalizar_lista_correos(receptor) if receptor else []

def _registrar_envio_correo(accion, destinatarios, ok, extra=""):
    dest_txt = ", ".join(_normalizar_lista_correos(destinatarios))
    estado = "OK" if ok else "FALLO"
    detalle = f"{dest_txt} → {estado}"
    if extra:
        detalle = f"{detalle} — {extra}"
    try:
        registrar_accion(accion, detalle[:250])
    except Exception:
        pass
...
def _enviar_correo_html(asunto, cuerpo_html, destinatarios, cc=None):
    ...
    if not conf:
        return False
    destinatarios = _normalizar_lista_correos(*destinatarios)
    if not destinatarios:
        return False
    cc_list = _normalizar_lista_correos(cc) if isinstance(cc, (list, tuple, set)) else _normalizar_lista_correos(cc)
    cc_list = [c for c in cc_list if c.lower() not in {d.lower() for d in destinatarios}]
    try:
        msg = MIMEMultipart()
        ...
        dest_txt = ", ".join(destinatarios + (cc_list or []))
        try:
            conn = conectar_db()
            f_h = hora_chile().strftime("%Y-%m-%d %H:%M:%S")
            conn.execute(
                ...
                (
                    st.session_state.get("email", "SISTEMA"),
                    "FALLO_SMTP",
                    f"{dest_txt} — {str(e)[:120]}",
                    f_h,
                ),
            )
            conn.commit()
...
def fecha_fin_prueba_demo(desde=None):
    return (desde or hoy) + timedelta(days=DEMO_DIAS_PRUEBA)

def usuario_prueba_vigente(fecha_expira):
    if not fecha_expira:
        return True
    try:
        return pd.to_datetime(fecha_expira).date() >= hoy
    except Exception:
        return True

def enviar_correo_invitacion_demo(email_nuevo, clave_plana, rol, admin_email, fecha_expira):
    from erp_correo_html import plantilla_correo_html

    f_h = hora_chile().strftime("%d-%m-%Y %H:%M")
    f_exp = pd.to_datetime(fecha_expira).strftime("%d-%m-%Y")
    perfil_txt = etiqueta_perfil_demo(rol, email_nuevo)
    interior = f"""
            <p style='color: #1F2933; line-height: 1.55;'>
                Has sido invitado a probar el <b>ERP Agrícola en entorno de demostración</b>.
                Este acceso es temporal y sirve para conocer el sistema sin afectar datos de producción.
            </p>
            <div style='background: #FFF8E1; border: 1px solid #FFE082; border-radius: 10px; padding: 14px 18px; margin: 18px 0;'>
                <p style='margin: 0; color: #5D4037; font-weight: 700;'>⏳ Periodo de prueba: {DEMO_DIAS_PRUEBA} días</p>
                <p style='margin: 8px 0 0; color: #5D4037;'>Su acceso estará vigente hasta el <b>{f_exp}</b> (inclusive).
                Recibirá avisos automáticos por correo <b>24 horas antes</b> y <b>al vencer el plazo</b>.</p>
            </div>
            <div style='background: #F3E5F5; border: 1px solid #CE93D8; border-radius: 10px; padding: 18px 20px; margin: 18px 0;'>
                ...
                <p style='margin: 6px 0;'><b>Usuario:</b> <span style='color: #6A1B9A; font-weight: bold;'>{email_nuevo}</span></p>
                <p style='margin: 6px 0;'><b>Contraseña:</b> <span style='color: #6A1B9A; font-weight: bold;'>{clave_plana}</span></p>
            ...
            <p style='font-size: 13px; color: #5F6B7A; margin: 6px 0;'><b>Invitación emitida por:</b> {admin_email or 'Administrador ERP Demo'}</p>
            <p style='font-size: 13px; color: #5F6B7A; margin: 6px 0;'><b>Fecha:</b> {f_h} (Chile UTC-4)</p>
    """
    cuerpo = plantilla_correo_html(
        "invitacion",
        "🚜 Bienvenido al ERP Agrícola — Ambiente Demo",
        interior,
        nombre_erp="ERP DEMO AGRICOLA",
        pie="Si no esperaba esta invitación, ignore el correo.",
    )
    cc_respaldo = _destinatarios_admin_demo(admin_email)
    cc_respaldo = [c for c in cc_respaldo if c.lower() != str(email_nuevo or "").strip().lower()]
    ok_invitado = _enviar_correo_html(
        f"🚜 Invitación — ERP Agrícola Demo (acceso de prueba {DEMO_DIAS_PRUEBA} días)",
        cuerpo,
        [email_nuevo],
    )
    _registrar_envio_correo("MAIL INVITACION", email_nuevo, ok_invitado)
    ok_copia = True
    if cc_respaldo:
        asunto_copia = f"📋 Copia — Invitación demo enviada a {email_nuevo}"
        if not ok_invitado:
            asunto_copia += " (falló envío al invitado)"
        ok_copia = _enviar_correo_html(asunto_copia, cuerpo, cc_respaldo)
        _registrar_envio_correo("MAIL INVITACION COPIA", cc_respaldo, ok_copia)
    return {"invitado": ok_invitado, "copia": ok_copia}
...
def _inicializar_db():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS gastos_campob (id INTEGER PRIMARY KEY AUTOINCREMENT, fecha DATE, documento TEXT, item TEXT, monto REAL)""")
    ...
    cols_usr = [c[1] for c in cursor.fetchall()]
    for col_usr, ddl in (
        ("fecha_expira", "ALTER TABLE usuarios ADD COLUMN fecha_expira DATE"),
        ("invitado_por", "ALTER TABLE usuarios ADD COLUMN invitado_por TEXT"),
        ("alerta_24h_enviada", "ALTER TABLE usuarios ADD COLUMN alerta_24h_enviada INTEGER DEFAULT 0"),
        ("alerta_vencido_enviada", "ALTER TABLE usuarios ADD COLUMN alerta_vencido_enviada INTEGER DEFAULT 0"),
        ("modulos", "ALTER TABLE usuarios ADD COLUMN modulos TEXT DEFAULT ''"),
    ):
        if col_usr not in cols_usr:
            try: cursor.execute(ddl)
            except: pass
    usuarios_seed = [
        ('osvaldolira@laconcepcion.cl', hash_password('9083'), 'super_admin'),
        ('secretaria@laconcepcion.cl', hash_password('9111'), 'operador'),
        ('demo@erpmaster.cl', hash_password('5678'), 'admin_cliente'),
        ('certificacion@erpmaster.cl', hash_password('9091'), 'certificacion'),
    ]
    n_usuarios = cursor.execute("SELECT COUNT(*) FROM usuarios").fetchone()[0]
    if n_usuarios == 0:
        for u, p, r in usuarios_seed:
            cursor.execute(
                "INSERT INTO usuarios (email, password, rol, fecha_expira, alerta_24h_enviada) VALUES (?,?,?,NULL,0)",
                (u, p, r),
            )
    else:
        cursor.execute(
            "INSERT OR IGNORE INTO usuarios (email, password, rol, fecha_expira, alerta_24h_enviada) VALUES (?,?,?,NULL,0)",
            ('osvaldolira@laconcepcion.cl', hash_password('9083'), 'super_admin'),
        )
    cursor.execute(
        "UPDATE usuarios SET password=?, rol='super_admin', fecha_expira=NULL WHERE email='osvaldolira@laconcepcion.cl'",
        (hash_password('9083'),),
    )
    cursor.execute(
        "UPDATE usuarios SET rol='admin_cliente' WHERE lower(email)='demo@erpmaster.cl'"
    )
    _migrar_roles_demo_v1(cursor)
    for u_perm in USUARIOS_DEMO_PERMANENTES:
        cursor.execute(
            "UPDATE usuarios SET fecha_expira=NULL, alerta_24h_enviada=0, alerta_vencido_enviada=0 WHERE email=?",
            (u_perm,),
        )
    migrar_nombres_cuarteles(cursor)
    migrar_gantt(cursor)
    ...
    if "razon_social" not in cols_fac:
        try:
            cursor.execute("ALTER TABLE facturas ADD COLUMN razon_social TEXT DEFAULT 'ERP Demo Agrícola'")
        except Exception:
            pass
...
def _migrar_roles_demo_v1(cursor):
    cursor.execute("CREATE TABLE IF NOT EXISTS schema_meta (clave TEXT PRIMARY KEY, valor TEXT)")
    if cursor.execute("SELECT 1 FROM schema_meta WHERE clave='roles_demo_v1'").fetchone():
        return
    cursor.execute(
        "UPDATE usuarios SET rol='super_admin' WHERE lower(email)='osvaldolira@laconcepcion.cl'"
    )
    cursor.execute("UPDATE usuarios SET rol='admin_cliente' WHERE rol='admin'")
    cursor.execute(
        "UPDATE usuarios SET rol='admin_cliente' WHERE lower(email)='demo@erpmaster.cl'"
    )
    cursor.execute(
        "INSERT OR REPLACE INTO schema_meta (clave, valor) VALUES ('roles_demo_v1', '1')"
    )

def migrar_nombres_cuarteles(cursor):
    tablas = [
    ...
    for tabla, columna in tablas:
        for antiguo, nuevo in MAPEO_CUARTELES_LEGACY.items():
            cursor.execute(
                f"UPDATE {tabla} SET {columna}=? WHERE UPPER(TRIM({columna}))=?",
                (nuevo, antiguo),
            )

def cargar_datos_demo(cursor):
    cursor.execute("CREATE TABLE IF NOT EXISTS schema_meta (clave TEXT PRIMARY KEY, valor TEXT)")
    if cursor.execute(
        "SELECT 1 FROM schema_meta WHERE clave=?", (DEMO_SEED_VERSION,),
    ).fetchone():
        return
    vaciar_datos_demo(cursor)
    h = hora_chile().date()
    mes = hora_chile().strftime("%m")
    anio = hora_chile().year
    sembrar_datos_demo(
        cursor, h, mes, anio, PRORRATEO_RRHH, CENTROS_COSTO, CUARTELES_PRORRATEO,
        TEMPORADAS_COSTOS, RAZONES_SOCIALES_COMPRAS[0], TIPO_GASTO_SIN_CLASIFICAR, hora_chile,
    )
    cursor.execute(
...
# =============================================================================
# 3. UTILIDADES PDF, INDICADORES Y CLIMA + INYECTOR CSS SEGREGADO QUIRÚRGICO v11.5.4
# =============================================================================
...
def _sumar_horas_frio_decimal(df, umbral_frio=UMBRAL_HORA_FRIO, desde=None):
    ...
    if desde is not None:
        ...
        work = work[work["fecha_hora"] >= desde]
    if work.empty:
        return 0.0
...
def widget_prueba_demo_compact(fecha_expira):
    """Contador de días restantes del periodo de prueba (reemplaza el antiguo widget de clima)."""
    dias = dias_restantes_prueba(fecha_expira)
    if dias is None or dias < 0:
        return
    f_exp = pd.to_datetime(fecha_expira).strftime("%d-%m-%Y")
    alerta = dias <= 3
    extra_cls = " clima-widget-alerta" if alerta else ""
    if dias == 0:
        dias_txt = "Hoy"
        label = "último día de prueba"
    elif dias == 1:
        dias_txt = "1"
        label = "día restante"
    else:
    ...
    meta = f"Vence el {f_exp}"
    if alerta:
        meta += " · contacte al admin para extender"
    st.markdown(
        f"""<div class="clima-widget-compact{extra_cls}" title="Periodo de prueba demo · {DEMO_DIAS_PRUEBA} días desde la invitación">
                <div class="clima-widget-kicker">⏳ Periodo de prueba</div>
                <div class="clima-widget-valor">{dias_txt}</div>
                <div class="clima-widget-label">{label}</div>
                <div class="clima-widget-meta">{meta}</div>
            </div>""",
        unsafe_allow_html=True,
    )

def encabezado_dashboard_con_clima():
    """Encabezado dashboard demo: dona usuarios solo super admin; widget prueba solo invitados."""
    ...
    titulo = "🚜 ERP DEMO AGRICOLA"
    sub_html = ""
    if subtitulo:
        sub_html = (
            f'<p style="text-align:left;color:#5F6B7A;font-size:0.92rem;'
            f'margin:0 0 0.35rem;">{subtitulo}</p>'
        )
    ...
    fecha_exp = None
    if email and email not in USUARIOS_DEMO_PERMANENTES:
        try:
            _conn_pr = conectar_db()
            _row_pr = _conn_pr.execute(
                "SELECT fecha_expira FROM usuarios WHERE email=?",
                (email,),
            ).fetchone()
            _conn_pr.close()
            if _row_pr and _row_pr[0]:
                fecha_exp = _row_pr[0]
        except Exception:
            pass

    if fecha_exp:
        col_tit, col_cli = st.columns([5, 2], gap="small")
        with col_tit:
            encabezado_modulo("Dashboard", "🚜 ERP DEMO AGRICOLA")
        with col_cli:
            widget_prueba_demo_compact(fecha_exp)
    else:
        encabezado_modulo("Dashboard", "🚜 ERP DEMO AGRICOLA")


def _render_widget_usuarios_dashboard_al_final(conn, email_viewer=None):
    """Donut de actividad de usuarios al pie del dashboard."""
    try:
        from erp_sesiones_usuarios import html_widget_actividad_usuarios
        widget_html = html_widget_actividad_usuarios(conn, email_viewer=email_viewer)
        if widget_html:
            st.divider()
            st.markdown(
...
def _pdf_dibujar_fila_tabla(pdf, valores, cols, anchos, estilo_celda_fn, row, h_line=4, header=False):
    x0 = pdf.get_x()
    y0 = pdf.get_y()
    font_size = 7 if header else 6
    n_lines = 1
...
def generar_pdf_petroleo_historial(df, saldo_petroleo=None):
    try:
        ...
        pdf.cell(0, 8, _pdf_txt("ERP DEMO AGRICOLA"), ln=1, align="C")
        if saldo_petroleo is not None:
            pdf.set_font("Helvetica", "B", 9)
            pdf.cell(0, 6, _pdf_txt(f"SALDO ESTANQUE: {f_decimal(saldo_petroleo)} L"), ln=1, align="R")
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 9, _pdf_txt("HISTORIAL DE PETRÓLEO"), ln=1, align="C")
        pdf.ln(4)
...
def generar_pdf_blob(df, titulo, incluir_precios=True, total_manual=None, modo_petroleo=False, orden_asc=True, saldo_petroleo=None, campo_suma_forzado=None, estilo_celda_fn=None):
    try:
        ...
        if modo_petroleo and saldo_petroleo is not None:
            ...
            pdf.cell(0, 6, _pdf_txt(f"SALDO ESTANQUE: {f_decimal(saldo_petroleo)} L"), ln=1, align="R")
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 9, _pdf_txt(titulo), ln=1, align="C")
        pdf.ln(4)
        df_p = _pdf_preparar_df(df)
        if orden_asc and 'fecha' in [c.lower() for c in df_p.columns]:
            cf = [c for c in df_p.columns if c.lower() == 'fecha'][0]
            df_p[cf] = pd.to_datetime(df_p[cf])
            df_p = df_p.sort_values(by=cf, ascending=True)
            df_p[cf] = df_p[cf].dt.date
        ...
        _pdf_dibujar_fila_tabla(pdf, [""] * len(cols), cols, anchos, estilo_celda_fn, None, h_line=8, header=True)
        for _, row in df_p.iterrows():
            _pdf_dibujar_fila_tabla(
                pdf, [row[c] for c in cols], cols, anchos, estilo_celda_fn, row, h_line=4,
            )
...
def generar_pdf_libro_campo(df):
    try:
        if df is None or df.empty:
            return None
        df_p = df.copy()
        ...
        pdf.cell(0, 7, _pdf_txt("REGISTRO FITOSANITARIO"), ln=1, align="C")
        pdf.ln(3)
...
def render_historial_tesoreria_agrupado(df, mostrar_resumen=True):
    ...
    for num, ((prov, f_pago, metodo), grp) in reversed(list(enumerate(grupos, start=1))):
        ...
        cols_det = ["nro_documento", "monto_total"]
        ren_det = {"nro_documento": "DOCUMENTO", "monto_total": "MONTO"}
        if "razon_social" in grp.columns:
            cols_det = ["nro_documento", "razon_social", "monto_total"]
            ren_det["razon_social"] = "RAZÓN SOCIAL"
        df_det = grp[cols_det].copy().rename(columns=ren_det)
...
def generar_pdf_tesoreria_pagos(df):
    try:
        ...
        df_p = df_p.sort_values(["fecha_pago", "proveedor", "metodo_pago", "nro_documento"], ascending=[True, True, True, True])
        grupos = list(df_p.groupby(["proveedor", "fecha_pago", "metodo_pago"], sort=False))
        con_rs = "razon_social" in df_p.columns
        ...
        pdf.cell(0, 8, _pdf_txt("ERP DEMO AGRICOLA"), ln=1, align="C")
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(0, 7, _pdf_txt("HISTORIAL DE PAGOS"), ln=1, align="C")
        pdf.ln(3)
        ...
        for num, ((prov, f_pago, metodo), grp) in reversed(list(enumerate(grupos, start=1))):
            ...
            for _, row in grp.iterrows():
                ...
                if con_rs:
                    pdf.cell(widths[1], 6, _pdf_txt(str(row.get("razon_social") or "ERP Demo"))[:28], border=1)
                    pdf.cell(widths[2], 6, _pdf_txt(f"${f_puntos(row['monto_total'])}"), border=1, align="R")
                else:
                    pdf.cell(widths[1], 6, _pdf_txt(f"${f_puntos(row['monto_total'])}"), border=1, align="R")
                pdf.ln()
            pdf.ln(2)
        ...
        pdf.cell(
            ancho_tabla, 5,
            _pdf_txt(f"Total: {len(grupos)} pago(s), {len(df_p)} documento(s) — N° {len(grupos)} = más reciente"),
            ln=1,
        )
        return pdf.output(dest="S").encode("latin-1")
    except Exception:
...
def inyectar_css():
    css_comun = """<style>
        ...
        :root {
            ...
            --radio: 14px;
            --clima-bg: linear-gradient(135deg, #F3E5F5 0%, #EDE7F6 100%);
            --clima-border: #CE93D8;
            --clima-kicker: #6A1B9A;
            --clima-valor: #4A148C;
            --clima-label: #7B1FA2;
...
""" + CSS_FORZAR_TEMA_CLARO + """
        ...
        .clima-widget-valor {
            font-size: 2rem;
            font-weight: 800;
            color: var(--clima-valor);
            line-height: 1;
        }
        .clima-widget-label {
            font-size: 0.76rem;
            color: var(--clima-label);
            margin-top: 0.12rem;
        }
        .clima-widget-meta {
            font-size: 0.72rem;
        ...
        .us-donut-hole {
            ...
            box-shadow: inset 0 0 0 1px rgba(106, 27, 154, 0.1);
        }
        .us-donut-center {
            position: absolute;
            top: 50%;
        ...
        .saldo-banner {
            ...
            color: var(--saldo-text);
            padding: 1rem 1.2rem;
            border-radius: var(--radio);
            border: 1px solid var(--saldo-border);
            text-align: center;
            margin-bottom: 1.2rem;
            font-size: 1.25rem;
            font-weight: 800;
        ...
        .alert-roja {
            background: var(--alert-roja-bg);
            color: var(--alert-roja-text);
            padding: 0.8rem 1rem;
            border-radius: 12px;
            border: 1px solid var(--alert-roja-border);
            margin-bottom: 0.8rem;
            font-weight: 700;
            text-align: center;
        }
        .alert-amarilla {
            background: var(--alert-amarilla-bg);
            color: var(--alert-amarilla-text);
            padding: 0.8rem 1rem;
            border-radius: 12px;
            border: 1px solid var(--alert-amarilla-border);
            margin-bottom: 0.8rem;
            font-weight: 700;
            text-align: center;
        }
        .alert-naranja {
            background: var(--alert-naranja-bg);
            color: var(--alert-naranja-text);
            padding: 0.8rem 1rem;
            border-radius: 12px;
            border: 1px solid var(--alert-naranja-border);
            margin-bottom: 0.8rem;
            font-weight: 700;
            text-align: center;
        }
    ...
    st.markdown(css_comun + f"<style>{CSS_MOBILE_RESPONSIVE}</style>", unsafe_allow_html=True)
    try:
...
def aplicar_tema_modulo(modulo_key):
    tema = TEMAS_MODULO.get(modulo_key, TEMAS_MODULO["DASHBOARD"]); st.markdown(
        f"""<style>
        :root {{
            --modulo-accent: {tema['color']};
            --modulo-claro: {tema['claro']};
        }}
        .modulo-titulo h1 {{ color: {tema['color']} !important; }}
        </style>""",
        unsafe_allow_html=True,
    )
...
def titulo_modulo(titulo, subtitulo=""):
    ...
    if st.session_state.get("modulo_activo"):
        tema = TEMAS_MODULO.get(st.session_state["modulo_activo"], {})
        if tema.get("sub") and not subtitulo:
            subtitulo = tema["sub"]
            sub = f"<p>{subtitulo}</p>"
    st.markdown(
        f"<div class='modulo-titulo'><h1>{titulo}</h1>{sub}{badge}</div>",
        unsafe_allow_html=True,
    )
...
def migrar_globalgap(cursor):
    ...
    if cursor.execute("SELECT COUNT(*) FROM gap_checklist").fetchone()[0] == 0:
        items = [
            ("AFB", "AFB-01", "Mapa actualizado del fundo con cuarteles, bodegas, baños y puntos de agua", 1),
...
def migrar_especie_globalgap(cursor):
    for tabla in (
        ...
        if "especie" not in cols:
            try:
                cursor.execute(f"ALTER TABLE {tabla} ADD COLUMN especie TEXT DEFAULT 'ESPECIE 1'")
            except Exception:
                pass
    cursor.execute(
    ...
    if cursor.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='gantt_proyectos'"
    ).fetchone():
        cursor.execute(
        ...
        cursor.execute(
            "UPDATE gantt_proyectos SET especie='ESPECIE 1' WHERE nombre LIKE '%ESPECIE 1%'"
        )
...
def fecha_cosecha_especie2_planificada():
...
def plantilla_calendario_gantt():
    ...
    cosecha_cir = fecha_cosecha_especie2_planificada()
    cierre_cir = cosecha_cir + timedelta(days=25)
    return {
        "Temporada ESPECIE 1 2026": {
        ...
        "Temporada ESPECIE 2 2026": {
            "desc": "Plan operativo cosecha y labores especie2",
            "fi": str(inicio),
            "ff": str(cierre_cir),
            "cc": "CUARTEL C",
            "resp": "Jefe de Campo",
            "esp": "ESPECIE 2",
            "tareas": [
                ("Calibración equipos especie2", inicio, inicio + timedelta(days=12), 0, "Taller", "Media"),
                ("Monitoreo sanitario y labores culturales especie2", inicio, inicio + timedelta(days=90), 0, "Cuadrilla B", "Alta"),
                ...
                ("Preparación logística cosecha especie2", cosecha_cir - timedelta(days=14), cosecha_cir - timedelta(days=1), 0, "Supervisor cosecha", "Alta"),
                ("Cosecha ESPECIE 2", cosecha_cir, cosecha_cir + timedelta(days=18), 0, "Supervisor cosecha", "Alta"),
...
def cuarteles_gap_especie(especie):
    return GAP_ESPECIE_CUARTELES.get(especie, []) + ["CUARTEL F"]
...
def seleccionar_especie_gap():
    ...
    labels = {"ESPECIE 1": "🟢 ESPECIE 1", "ESPECIE 2": "🟣 ESPECIE 2"}
    return st.radio(
        "Cultivo activo",
        GAP_ESPECIES,
        horizontal=True,
...
def migrar_gantt(cursor):
    cursor.execute("""CREATE TABLE IF NOT EXISTS gantt_proyectos (
        ...
        responsable TEXT,
        estado TEXT DEFAULT 'Activo',
        especie TEXT DEFAULT 'ESPECIE 1'
    )""")
    cursor.execute("PRAGMA table_info(gantt_proyectos)")
    cols_gantt = [c[1] for c in cursor.fetchall()]
    if "especie" not in cols_gantt:
        try:
            cursor.execute("ALTER TABLE gantt_proyectos ADD COLUMN especie TEXT DEFAULT 'ESPECIE 1'")
        except Exception:
            pass
...
def gantt_evaluar_alerta(avance_real, avance_esperado, fecha_fin, umbrales=None):
...
def cargar_tareas_gantt(conn, proyecto_id=None, especie=None):
    q = """
        SELECT t.id, p.nombre as proyecto, t.actividad, t.fecha_inicio, t.fecha_fin,
               ...
               COALESCE(p.especie, 'ESPECIE 1') as especie
        FROM gantt_tareas t
        JOIN gantt_proyectos p ON p.id = t.proyecto_id
        WHERE p.estado = 'Activo'
    """
    if especie:
        q += f" AND (COALESCE(p.especie,'ESPECIE 1') = '{especie}' OR COALESCE(p.especie,'ESPECIE 1') = '{GAP_ESPECIE_GENERAL}')"
    if proyecto_id:
    ...
    for _, row in df.iterrows():
        ...
        al = gantt_evaluar_alerta(row["avance_pct"], esp, row["fecha_fin"], umbrales)
...
def _auditar_bodega_pppl(conn):
    ...
    for inv_id, producto, familia, pppl_bodega, phi_bodega in conn.execute(
        "SELECT id, producto, familia, COALESCE(pppl_aprobado,0), COALESCE(dias_carencia,0) FROM inventario ORDER BY familia, producto"
    ):
...
def resumen_globalgap(conn, especie="ESPECIE 1"):
    total_chk = conn.execute("SELECT COUNT(*) FROM gap_checklist").fetchone()[0]
    cumple = conn.execute(
        "SELECT COUNT(*) FROM gap_evaluacion WHERE estado='Cumple' AND COALESCE(especie,'ESPECIE 1')=?",
        (especie,),
    ).fetchone()[0]
    cuarteles = cuarteles_gap_especie(especie)
    placeholders = ",".join("?" * len(cuarteles))
    nc_abiertas = conn.execute(
        f"SELECT COUNT(*) FROM gap_nc WHERE estado='Abierta' AND (COALESCE(especie,'ESPECIE 1')=? OR cuartel IN ({placeholders}))",
        (especie, *cuarteles),
    ).fetchone()[0]
    pppl = conn.execute(
...
def panel_planificacion_gantt(conn, especie):
    inicio = inicio_gestion_gantt()
    cosecha_esp = fecha_cosecha_especie2_planificada() if especie == "ESPECIE 2" else fecha_cosecha_planificada(inicio.year)
    ...
    if sec_gantt == _gantt_secc[0]:
        proys = pd.read_sql_query(
            f"SELECT id, nombre FROM gantt_proyectos WHERE estado='Activo' AND (COALESCE(especie,'ESPECIE 1')='{especie}' OR COALESCE(especie,'ESPECIE 1')='{GAP_ESPECIE_GENERAL}') ORDER BY nombre",
            conn,
        )
        ...
        if sec_gest == _gest_secc[0]:
            with st.form("gantt_new_task", clear_on_submit=True):
                proy_opts = pd.read_sql_query(
                    f"SELECT id, nombre FROM gantt_proyectos WHERE COALESCE(especie,'ESPECIE 1') IN ('{especie}', '{GAP_ESPECIE_GENERAL}') ORDER BY nombre",
                    conn,
                )
                if proy_opts.empty:
...
def modulo_globalgap():
    ...
    if sec_gap == _gap_secc[0]:
        ...
        st.divider()
        _render_auditoria_pppl_bodega(conn, key_prefix=f"gap_pppl_{especie[:6]}", especie=especie)

    elif sec_gap == _gap_secc[1]:
        with st.form("gap_doc_new", clear_on_submit=True):
            d_tipo = st.selectbox("Tipo", ["Manual BPA", "Mapa fundo", "Política integrada", "Análisis agua", "Procedimiento", "Otro"])
        ...
        df_doc = pd.read_sql_query(
            "SELECT tipo as TIPO, titulo as TITULO, version as VER, fecha_vigencia as VIGENTE, responsable as RESPONSABLE, notas as NOTAS FROM gap_documentos WHERE COALESCE(especie,'ESPECIE 1')=? ORDER BY fecha_registro DESC",
            conn,
            params=(especie,),
        )
        ...
        q_chk = f"""SELECT c.id, c.codigo, c.capitulo, c.descripcion, COALESCE(e.estado,'Pendiente') as estado, e.fecha_revision, e.responsable
                    ...
                    LEFT JOIN gap_evaluacion e ON c.id=e.checklist_id AND COALESCE(e.especie,'ESPECIE 1')='{especie}'"""
        if filtro_cap != "TODOS":
        ...
        with st.form("gap_eval_form"):
            ...
            if st.form_submit_button("GUARDAR EVALUACIÓN") and sel:
                ...
                conn.execute("DELETE FROM gap_evaluacion WHERE checklist_id=? AND COALESCE(especie,'ESPECIE 1')=?", (chk_id, especie))
                conn.execute(
        ...
        df_nc = pd.read_sql_query(
            f"SELECT codigo, capitulo, descripcion, accion_correctiva, plazo, estado, cuartel FROM gap_nc WHERE COALESCE(especie,'ESPECIE 1')=? OR cuartel IN ({','.join('?' * len(cuarteles_esp))}) ORDER BY fecha_apertura DESC",
            conn,
        ...
        pdf_globalgap(df_cap, "Capacitaciones", especie, "capacitaciones", detalle="Fundo", estilo_celda_fn=_pdf_estilo_cap_vencida)
        ...
        pdf_globalgap(df_agua, "Análisis de agua", especie, "agua", detalle="Fundo", estilo_celda_fn=_pdf_estilo_gap_agua)
        ...
        pdf_globalgap(df_cal, "Calibración equipos", especie, "calibracion", detalle="Fundo")
...
def encabezado_modulo(modulo_key, titulo, subtitulo=None):
    ...
    sub = f"<p>{subtitulo}</p>" if subtitulo else ""
    st.markdown(
        f"""<div class='modulo-titulo'>
            <h1>{titulo}</h1>{sub}
            <span class='modulo-badge'>{modulo_key.upper()}</span>
        </div>""",
        unsafe_allow_html=True,
    )
...
def render_manual_viewer(html_body, height=680):
    ...
    <style>
      ...
      blockquote {{ background:#F1F8E9; border-left:4px solid #2E7D32; padding:10px 14px; margin:12px 0; }}
    ...
    components.html(
        ...
        f"sandbox='' title='Manual ERP Demo'></iframe>",
        height=height + 12,
    )

def _modulos_usuario_actual():
    rol = st.session_state.get("rol", "operador")
    email = st.session_state.get("email", "")
    modulos = list(construir_menu_usuario(email, rol).values())
    if puede_administracion() and "Administracion" not in modulos:
        modulos.append("Administracion")
    return modulos, _rol_sesion(), email

def modulo_manual():
    encabezado_modulo("Manual", "📖 MANUAL DE USUARIO")
    if es_certificacion():
        st.info("Perfil certificación: consulte el manual de módulos GlobalGAP, Libro de Campo y Bodega.")
        guia = manual_contenido.GUIA_RAPIDA_CERT_HTML
        completo = manual_contenido.MANUAL_COMPLETO_CERT_HTML
    else:
        st.markdown(
            """<div style="background:#FFF8E1;border:1px solid #FFE082;color:#5D4037;
            padding:0.75rem 1rem;border-radius:10px;margin-bottom:1rem;font-weight:600;font-size:0.92rem;">
            🔒 Documento demo — solo visualización dentro del sistema. No disponible para descarga.</div>""",
            unsafe_allow_html=True,
        )
        if es_admin_cliente() and not es_super_admin():
            st.info(
                "Perfil **Administrador de campo**: en el manual completo revise la sección "
                "**Entorno demo** y **Administración demo** (permisos separados del super administrador)."
            )
        elif es_super_admin():
            st.caption(
                "Super administrador: incluye pestañas reservadas (Bitácora, Usuarios, Respaldo, Plataforma demo)."
            )
    ...
    if sec_man == _man_secc[0]:
        ...
        render_manual_viewer(completo, height=680)
...
def _imputar_costos_rrhh(conn, trabajador_id, mes, anio, total):
    ...
    try:
        ...
        pass
    for cc_interno, porcentaje in cargar_prorrateo_cc(conn).items():
        parte_costo = total * porcentaje
        if parte_costo > 0:
            try:
                conn.execute(
...
def _registrar_servicio_contratista(
    ...
    if not selcc:
        return False, "Seleccione al menos un cuartel."
    parte = imp / len(selcc)
    conn.execute(
...
def modulo_dashboard():
    ...
    st.divider()
    dfr_base, _ = _armar_dataframe_costos_dashboard(conn, CUARTELES_OFICIALES, PRORRATEO_RRHH)
    if not dfr_base.empty:
        df_gastos = _build_dashboard_gastos_cc_df(conn, dfr_base)
        _render_dashboard_cc_y_pagos(df_gastos, df_f)

    if es_super_admin():
        _render_widget_usuarios_dashboard_al_final(
            conn, email_viewer=str(st.session_state.get("email", "") or "").strip().lower()
        )
    conn.close()
def modulo_petroleo():
    conn = conectar_db()
    ...
    _pet_secc = ["🚜 SALIDA", "📊 HISTORIAL", "📋 PLANILLA MAESTRA"]
    sec_pet = nav_seccion(_pet_secc, "pet_nav", "Sección")
    st.info(
        "La **carga al estanque** se registra en **Compras → INGRESO** "
        "(casilla *Compra de petróleo* + litros y monto). Aquí solo se despacha a cuarteles."
    )
...
def modulo_compras():
    ...
    if sec_comp == _comp_secc[0]:
        ...
        if not es_agro and sin_doc:
            nro = c1.text_input("N° Doc", value="AUTOGENERADO", disabled=True, key="comp_nro_inp")
        else:
            nro = c1.text_input("N° Factura / Doc", value=st.session_state['comp_nro'], key="comp_nro_inp")
        ...
        if es_agro:
            ...
            if st.session_state.get('car'):
                ...
                if st.button("💾 GUARDAR FACTURA COMPLETA", key="comp_btn_save"):
                    if nro.strip() == "" or not (prov or "").strip():
                        ...
                        conn.execute(
                            "INSERT INTO facturas (nro_documento, proveedor, fecha_compra, fecha_vencimiento, monto_total, concepto) VALUES (?,?,?,?,?,?)",
                        ...
                        conn.commit()
                        st.session_state['car'] = []
                        _limpiar_formulario_compras_ingreso()
            ...
            st.info(
                "⛽ **Petróleo (demo)** — sin cuarteles ni tipo de gasto. "
                "Litros + monto cargan el estanque al guardar; la factura queda en Tesorería."
            )
            with st.form("gv_petroleo_form", clear_on_submit=True):
                litros_pet = st.number_input("Litros cargados al estanque", 0.0, key="gv_litros")
                razon_gv = st.selectbox("Razón social", RAZONES_SOCIALES_COMPRAS, key="gv_razon")
                concepto_det = st.text_input("Detalle / Concepto", key="gv_pet_concepto")
                mt = st.number_input("Total factura ($)", 0.0, key="gv_pet_monto")
                if st.form_submit_button("💾 GUARDAR COMPRA PETRÓLEO"):
                    ...
                    if sin_doc:
                        ...
                        ng_final = nro.strip()
                    litros_pet = float(litros_pet or 0)
                    if pg == "":
                        ...
                        st.error("❌ Error: Debe ingresar los litros cargados al estanque.")
                    else:
                        conn.execute(
                        ...
                        conn.execute(
                            ...
                            ("Carga", litros_pet, pg.strip(), mt, str(fg1)),
                        )
                        _recalcular_imputacion_salidas_petroleo(conn)
                        conn.commit()
                        registrar_accion("GASTO PETROLEO", ng_final)
                        registrar_accion("PETROLEO", f"Carga {litros_pet}L — {ng_final}")
                        st.success(
                            f"✅ Compra de petróleo registrada ({ng_final}): "
                            f"{f_decimal(litros_pet)} L cargados al estanque. Queda en Tesorería."
                        )
                        _limpiar_formulario_compras_ingreso()
                        st.rerun()
            ...
            with st.form("gv_form", clear_on_submit=True):
                ...
                if st.form_submit_button("💾 GUARDAR GASTO OPERACIONAL"):
                    ...
                    if pg == "":
                        ...
                        conn.execute(
                            "INSERT INTO facturas (nro_documento, proveedor, fecha_compra, fecha_vencimiento, monto_total, tipo, concepto, razon_social, tipo_gasto) VALUES (?,?,?,?,?,?,?,?,?)",
                            (ng_final, pg, str(fg1), str(fg2), mt, "Gasto Operacional", concepto_det.strip(), razon_gv, tg_gv),
                        )
                        for c in selcc:
                            conn.execute(
                                ...
                                (ng_final + "_P", pg, str(fg1), str(fg2), 0, "Gasto Operacional", c.upper(), imp / len(selcc), concepto_det.strip(), razon_gv, tg_gv),
                            )
                        conn.commit()
        ...
        query_hist = f"""SELECT id as ID, nro_documento as [N° DOCUMENTO], proveedor as PROVEEDOR,
                                IFNULL(razon_social, 'ERP Demo Agrícola') as [RAZÓN SOCIAL],
        ...
        if not dfh_filtrado.empty:
            boton_pdf("PDF HISTORIAL COMPRAS", generar_pdf_blob(dfh_filtrado.drop(columns=["ID"], errors="ignore"), "HISTORIAL CONSOLIDADO DE COMPRAS", campo_suma_forzado="MONTO BRUTO"), "historial_compras.pdf", key="ch_pdf_dinamico")
...
def modulo_tesoreria():
    encabezado_modulo("Tesoreria", "💸 TESORERÍA")
    conn = conectar_db()
    
    def enviar_correo_pago_interno(proveedor, documentos, monto_total, metodo, usuario_operador):
        from erp_correo_html import plantilla_correo_html
        ...
        return _enviar_correo_html(
            f"🚨 [{NOMBRE_ERP}] Pago Procesado: {proveedor} ({n_docs} doc.)",
            cuerpo_html,
            destinatarios,
        )
    teso_secciones = [
    ...
    if sec_teso == teso_secciones[0]:
        ...
        dfp_show = dfp.drop(columns=["id"]).rename(columns={
        ...
        boton_pdf("PDF PENDIENTES", generar_pdf_blob(dfp_show, "DEUDAS PENDIENTES", campo_suma_forzado="SALDO", estilo_celda_fn=_pdf_estilo_tesoreria_vencida), "pendientes.pdf", key="t_pdf_1")
        ...
        if not prvs.empty:
            ...
            boton_pdf(
                f"PDF DEUDA {psel}",
                generar_pdf_blob(dfpr_show, f"DEUDA {psel}", campo_suma_forzado="saldo_pendiente", estilo_celda_fn=_pdf_estilo_tesoreria_vencida),
                f"deuda_{psel}.pdf",
                key="t_pdf_2",
            )
            if not dfpr.empty:
                ...
                with st.form("pago_proveedor_multi", clear_on_submit=False):
                    ...
                    enviar_mail_pago = st.checkbox(
                        "📧 Enviar correo de respaldo al equipo",
                        value=True,
                        key="t_prov_mail",
                        help="Notifica por correo a quienes tienen Mail respaldo Tesorería y a los correos configurados en el sistema.",
                    )
                        ...
                        if not ids_pagar:
                            ...
                            if lineas:
                                ...
                                with st.spinner("Despachando avisos..."):
                                    if enviar_mail_pago:
                                        mail_ok = enviar_correo_pago_interno(
                                            proveedor=psel,
                                            documentos=lineas,
                                ...
                                if enviar_mail_pago:
                                    if mail_ok:
                                        msg += " Correo de respaldo enviado al equipo."
                                    else:
                                        msg += " Pago registrado; no se pudo enviar el correo al equipo."
                            ...
                            if not ok:
                                ...
                                if enviar_mail_abono:
                                    if mail_ok:
                                        ...
                                        msg += " No se pudo enviar el correo al equipo."
                                msg += mensaje_avisos_pago_proveedor(
                                    conn, psel, mail_prov_ok, mail_prov, wa_ok, wa_dest, wa_err
                                )
                                st.success(msg)
...
def modulo_bodega():
    ...
    if es_certificacion():
        ...
        if sec_bod == _bod_cert[0]:
            ...
            pdf_globalgap(dfs, "Stock bodega", st.session_state.get("gap_especie", "ESPECIE 1"), "bodega_stock")
        conn.close()
        return
    _bod_secc = [
            ...
            if st.form_submit_button("REGISTRAR APERTURA DE STOCK"):
                if np.strip() != "":
                    ...
                    if existe:
                        ...
                        poblar_ingredientes_inventario(conn, cur.lastrowid)
                        conn.commit(); registrar_accion("BODEGA", f"Apertura stock {np}"); st.success("✅ Apertura de inventario registrada."); st.rerun()
...
def modulo_campob():
    encabezado_modulo("Campob", "🏡 CAMPO B")
    conn = conectar_db()
    nombre, fi, ff = nav_temporada(TEMPORADAS_ESPINO, "campob_nav", hoy=hoy)
    _render_temporada_gastos_sector(
        conn, nombre, fi, ff, "gastos_campob", "CAMPO B", "CAMPO B",
        "GUARDAR REGISTRO CAMPO B", f"cb_{nombre}",
    )
    conn.close()
...
def _mostrar_alertas_cruce_lc_bodega():
    ...
    if lc:
        filas = "".join(
            ...
            f"{html_lib.escape(f_cantidad(p['gasto_total']))} {html_lib.escape(p.get('um_gasto', 'gr'))}</li>"
...
def _insertar_linea_libro_campo(conn, fe_app, n_app, huerto, especie, item, total_agua, aplicador, op_cert, maquinaria, tractor):
    ...
    conn.execute(
        ...
        (fecha, n_aplicacion, sector, especie, producto, ingrediente, dosis, unidad_dosis,
         gasto_total, vol_total, tractor, maquina, aplicadores, fecha_viable, lote_producto,
         operador_certificado, unidad_gasto)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            str(fe_app), n_app, huerto.upper(), especie.strip(), item["producto"].strip(),
...
def modulo_libro_campo():
    ...
    sub_tabs = ["📥 INGRESO APLICACIÓN", "📜 HISTORIAL AUDITABLE", "⚠️ DESFASE BODEGA"]
    if es_admin():
    ...
    if sec_lc == sub_tabs[0]:
        ...
        especie = c1.selectbox("Especie", GAP_ESPECIES, key="lca_3")
        ...
        with st.form("lc_add_producto", clear_on_submit=True):
            ...
            if prod_nom and not producto_pppl_aprobado(conn, prod_nom):
                st.warning("Producto no está en PPPL GlobalGAP. Revise en GlobalGAP → PPPL antes de aplicar.")
    ...
    elif sec_lc == sub_tabs[2]:
        _tab_desface_lc_bodega(conn, "lc_desf")

    elif es_admin() and len(sub_tabs) > 3 and sec_lc == sub_tabs[3]:
            ...
            if not df_mod_list.empty:
                ...
                with st.form("form_edit_libro"):
                    ...
                    e_especie = ce1.selectbox(
                        "Modificar Especie",
                        GAP_ESPECIES,
                        index=GAP_ESPECIES.index(r_act["especie"]) if r_act["especie"] in GAP_ESPECIES else 0,
                    )
    ...
    if sec_rh == _rh_secc[0]:
        with st.form("rh_p", clear_on_submit=True):
            ...
            if st.form_submit_button("REGISTRAR NUEVO TRABAJADOR"):
                ...
                if n.strip() == "":
                    ...
                    conn.commit(); registrar_accion("RRHH", n); st.success("✅ Trabajador registrado."); st.rerun()
        df_p = pd.read_sql_query("SELECT * FROM personal", conn); st.dataframe(df_p, use_container_width=True)
        if st.session_state['email'] == 'osvaldolira@laconcepcion.cl' and not df_p.empty:
            ...
            if col1.button("✏️ MODIFICAR REGISTRO PERSONAL"):
                if st.text_input("Master", type="password", key="rh_p1") == CLAVE_MAESTRA:
                    ...
                    if not ok_rut:
                        ...
                        conn.commit(); registrar_accion("UPDATE RRHH", un); st.success("✏️ Ficha actualizada."); st.rerun()
            if col2.button("🗑️ ELIMINAR TRABAJADOR"):
                if st.text_input("Master", type="password", key="rh_p2") == CLAVE_MAESTRA:
                    ...
                    conn.commit(); registrar_accion("DELETE RRHH", isel['nombre']); st.warning("🗑️ Trabajador borrado."); st.rerun()
        ...
        if sec_rh_ct == _rh_ct_secc[0]:
            ...
            if not ct_rows_act:
                ...
                ct_cta = st.selectbox("Contratista", list(ct_map.keys()), key="rh_ct_cta_sel")
                ...
                fi_cta = cta1.date_input("Desde", hoy - timedelta(days=365), key="rh_ct_cta_fi")
        ...
        if not df_act.empty:
            ...
            if not df_prov.empty:
                ...
                if st.button("💾 GUARDAR PLANILLA DEL MES", key=f"remu_save_{prov_m}_{prov_a}"):
                    errores = []
                ...
                boton_pdf(
                    ...
                    generar_pdf_blob(
                        edited, f"PROVISIÓN {prov_m}/{prov_a}", campo_suma_forzado="LIQUIDO_A_PROVISIONAR",
                    ),
...
def _panel_corregir_compras_historial(conn, dfh):
    if dfh is None or dfh.empty or not es_admin():
        return
    st.divider()
    ...
    fk = f"comp_cf_{idm}"
    razon_ini = str(fila[8] or "ERP Demo Agrícola").strip()
...
def _lineas_rrhh_detalle_cc(conn, cc, fi, ff, prorrateo_rrhh):
    ...
    for (anio, mes_str), total_mes in sorted(totales.items()):
        parte = int(float(total_mes) * pct)
        if parte > 0:
            lineas.append({
                "Fecha": f"{anio}-{mes_str}-01",
                "Fuente": "RRHH",
                "Detalle": f"Prorrateo RRHH {mes_str}/{anio}",
                "Monto": parte,
            })
    return lineas
...
def _obtener_detalle_gastos_cc(conn, cc, prorrateo_rrhh, fi=None, ff=None, fi_rrhh=None, ff_rrhh=None):
    ...
    if not df_rr.empty:
        ...
        if "Rubro" in df_rr.columns:
            df_rr.loc[df_rr["Rubro"].astype(str).str.upper() == "RRHH", "Rubro"] = "RRHH de la casa"
        partes.append(df_rr[["Fecha", "Rubro", "Detalle", "Monto"]])
    df = pd.concat(partes, ignore_index=True) if partes else pd.DataFrame(columns=["Fecha", "Rubro", "Detalle", "Monto"])
    if df.empty:
        return df
...
def _render_temporada_costos(conn, temporada, fi, ff, cuarteles, prorrateo_rrhh, key_prefix):
    ...
    if sec_mat == sub_labels[0]:
        ...
        _mostrar_detalle_cc_temporada(
                ...
                f"{key_prefix}_cc{i}", prorrateo_rrhh,
...
def _render_flujo_temporada(conn, nombre, fi, ff, key_prefix):
    ...
    if df_flujo.empty:
        ...
        df_show = df_flujo[
            ["MES", "INGRESOS", "RRHH", "TESORERIA", "EGRESOS_REAL", "EGRESOS_PROY", "EGRESOS_TOTAL", "RESULTADO_MES", "EERR_ACUM"]
        ].copy()
        n_meses = len(df_show)
        df_show = pd.concat(
        ...
        styler = styler.apply(_style_col_flujo_proy(df_flujo, "TESO_PROY", "TESORERIA", n_meses), subset=["TESORERÍA"])
...
def modulo_maquinaria():
        ...
        with st.form("form_registro_maquinaria", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 GUARDAR EN BITÁCORA"):
                if not cod_maquina or detalle.strip() == "" or responsable.strip() == "":
                    ...
                    conn.commit()
                    registrar_accion("MAQUINARIA", f"Registro {cod_unico} - {id_maquina}"); st.success(f"✅ Evento guardado con éxito bajo el código: {cod_unico}");                     st.rerun()
    ...
    conn.close()
def _seccion_administracion_demo():
    """Sección activa de Administración según rol (persiste entre reruns)."""
    defs = [
        ("bitacora", "📜 BITÁCORA", es_super_admin()),
        ("usuarios", "👤 USUARIOS Y PERFILES", es_super_admin()),
        ("modulos", "🔐 MÓDULOS OPERADOR", True),
        ("familias", "🏷️ FAMILIAS PRODUCTO", True),
        ("maquinaria", "🚜 MAESTRA MAQUINARIA", True),
        ("proveedores", "🏢 MAESTRA PROVEEDORES", True),
        ("encargados", "💵 ENCARGADOS COMPRAS", True),
        ("ppto", "🍒 PPTO Y PRODUCCIÓN", True),
        ("flujo", "💰 INGRESOS FLUJO", True),
        ("respaldo", "💾 RESPALDO DATOS", es_super_admin()),
        ("plataforma", "🔧 PLATAFORMA DEMO", es_super_admin()),
    ]
    visibles = [(k, lbl) for k, lbl, ok in defs if ok]
    labels = [lbl for _, lbl in visibles]
    sec_label = nav_seccion(labels, "adm_sec_nav", "Sección de administración")
    return next(k for k, lbl in visibles if lbl == sec_label)

def _render_admin_plataforma_demo(conn):
    """Pestaña exclusiva super_admin: infraestructura del entorno demo."""
    st.markdown("#### Plataforma demo (ERP Master)")
    st.caption(
        "Gestión de infraestructura del andén demo: usuarios permanentes, versión de datos ficticios "
        "y herramientas de mantenimiento. No visible para administradores de campo."
    )
    n_super, n_cliente = contar_roles_admin_demo(conn)
    c1, c2, c3 = st.columns(3)
    c1.metric("Super administradores", n_super)
    c2.metric("Admins. de campo", n_cliente)
    c3.metric("Días de prueba", DEMO_DIAS_PRUEBA)
    st.markdown("##### Usuarios del entorno")
    df_u = pd.read_sql_query(
        """SELECT email, COALESCE(rol, 'operador') AS rol, fecha_expira, invitado_por
           FROM usuarios ORDER BY email""",
        conn,
    )
    if not df_u.empty:
        df_u["Perfil"] = df_u.apply(
            lambda r: etiqueta_perfil_demo(normalizar_rol_usuario(r["rol"], r["email"])),
            axis=1,
        )
        df_u["Vigencia"] = df_u["fecha_expira"].apply(
            lambda v: "Permanente" if not v else str(v)[:10]
        )
        st.dataframe(
            df_u[["email", "Perfil", "Vigencia", "invitado_por"]].rename(
                columns={"email": "Usuario", "invitado_por": "Invitado por"}
            ),
            use_container_width=True,
            hide_index=True,
        )
    st.markdown("##### Datos ficticios compartidos")
    seed_ok = conn.execute(
        "SELECT valor FROM schema_meta WHERE clave=?", (DEMO_SEED_VERSION,),
    ).fetchone()
    st.info(
        f"Versión de seed: **{DEMO_SEED_VERSION}** · "
        f"Estado: **{'sembrado' if seed_ok else 'pendiente'}** · "
        f"Base: `{NOMBRE_DB}`"
    )
    st.warning(
        "Re-sembrar borra movimientos, facturas, inventario demo y vuelve a cargar datos ficticios. "
        "No elimina usuarios ni su configuración de administración."
    )
    if st.button("🔄 RESEMBRAR DATOS FICTICIOS", type="primary", key="demo_reseed_btn"):
        st.session_state["demo_reseed_confirm"] = True
    if st.session_state.get("demo_reseed_confirm"):
        conf = st.checkbox(
            "Confirmo re-sembrar todos los datos operativos del demo",
            key="demo_reseed_chk",
        )
        if st.button("EJECUTAR RE-SEED", key="demo_reseed_go") and conf:
            cur = conn.cursor()
            ...
            sembrar_datos_demo(
                cur,
                h,
                hora_chile().strftime("%m"),
                hora_chile().year,
                PRORRATEO_RRHH,
                CENTROS_COSTO,
                CUARTELES_PRORRATEO,
                TEMPORADAS_COSTOS,
                RAZONES_SOCIALES_COMPRAS[0],
                TIPO_GASTO_SIN_CLASIFICAR,
                hora_chile,
            )
            cur.execute(
            ...
            conn.commit()
            registrar_accion("DEMO RESEED", DEMO_SEED_VERSION)
            st.session_state.pop("demo_reseed_confirm", None)
            st.success("Datos ficticios re-sembrados.")
            st.rerun()

def modulo_seguridad():
    if not puede_administracion():
        st.error("Solo administradores pueden acceder a Administración.")
        st.stop()
    encabezado_modulo("Administración", "⚙️ ADMINISTRACIÓN")
    if es_super_admin():
        st.caption(
            "**Super administrador (plataforma):** además de la administración del demo, "
            "tiene las pestañas **Bitácora**, **Usuarios**, **Respaldo** y **Plataforma demo**."
        )
    else:
        st.caption(
            "**Administrador de campo (vista demo):** maestras, presupuesto y módulos operador. "
            "Bitácora, **usuarios** y **respaldo de datos** se habilitan en la inducción del cliente real."
        )
    conn = conectar_db()
    sec = _seccion_administracion_demo()
    if sec == "bitacora":
        c1, c2 = st.columns(2)
        ...
        boton_pdf("PDF BITACORA", generar_pdf_blob(dfb, f"ADMINISTRACIÓN ({fi} a {ff})", incluir_precios=False), "administracion.pdf", key="s_pdf_f")
            ...
            emails = _form_mail_tesoreria_usuarios(conn, incluir_vigencia=True)
            st.caption(
                f"Usuarios nuevos: **{DEMO_DIAS_PRUEBA} días** de prueba. Perfil **Administrador de campo** permite "
                f"probar este módulo completo (como lo haría un cliente real). "
                f"**Super administrador** es solo plataforma ERP Master. "
                f"Perfil **certificacion**: GlobalGAP, Libro de Campo y Bodega (PPPL)."
            )
            perfiles_nuevo = list(perfiles_asignables_demo())
            with st.form("seg_nuevo_usuario", clear_on_submit=True):
                ...
                nr = st.selectbox(
                    "Perfil",
                    perfiles_nuevo,
                    format_func=etiqueta_perfil_demo,
                    key="seg_nuevo_perfil",
                )
                mail_teso_nuevo = st.checkbox("Mail respaldo Tesorería", key="seg_nuevo_mail_teso")
                if st.form_submit_button("CREAR USUARIO"):
                    if nu.strip() and np.strip():
                        if nr == "super_admin" and not es_super_admin():
                            st.error("Solo el super administrador puede crear ese perfil.")
                        else:
                            try:
                                email_nuevo = nu.strip().lower()
                                fecha_fin = fecha_fin_prueba_demo()
                                admin_inv = st.session_state.get("email", "")
                                conn.execute(
                                    """INSERT INTO usuarios (email, password, rol, fecha_expira, invitado_por, alerta_24h_enviada, alerta_vencido_enviada, mail_tesoreria)
                                       VALUES (?,?,?,?,?,0,0,?)""",
                                    (email_nuevo, hash_password(np), nr, str(fecha_fin), admin_inv, 1 if mail_teso_nuevo else 0),
                                )
                                conn.commit()
                                registrar_accion("USUARIO NUEVO", f"{email_nuevo} ({nr}) hasta {fecha_fin}")
                                clave_envio = np.strip()
                                mail_res = enviar_correo_invitacion_demo(
                                    email_nuevo, clave_envio, nr, admin_inv, fecha_fin,
                                )
                                ok_invitado = mail_res.get("invitado") if isinstance(mail_res, dict) else bool(mail_res)
                                ok_copia = mail_res.get("copia", False) if isinstance(mail_res, dict) else False
                                respaldos = _destinatarios_admin_demo(admin_inv)
                                msg = f"Usuario {email_nuevo} creado. Vigencia hasta {fecha_fin.strftime('%d-%m-%Y')}."
                                if ok_invitado:
                                    msg += f" Correo de invitación enviado a **{email_nuevo}**"
                                    if ok_copia and respaldos:
                                        msg += f" (copia enviada a {', '.join(respaldos)})."
                                    else:
                                        msg += "."
                                elif ok_copia and respaldos:
                                    msg += (
                                        f" **No se pudo enviar al invitado** {email_nuevo}, "
                                        f"pero sí la copia al admin ({', '.join(respaldos)}). Revise bitácora → MAIL INVITACION / FALLO_SMTP."
                                    )
                                elif not _conf_smtp_demo():
                                    msg += " No se pudo enviar: falta configuración SMTP en `/root/demo/.streamlit/secrets.toml`."
                                else:
                                    msg += f" No se pudo enviar el correo a **{email_nuevo}** ni copia al admin (revise bitácora → FALLO_SMTP)."
                                st.success(msg)
                                st.rerun()
                            except sqlite3.IntegrityError:
            ...
            if not emails:
                emails = []
            emails_gestion = [e for e in emails if usuario_gestionable_demo(conn, e)]
            if emails_gestion:
                perfiles_cambio = list(perfiles_asignables_demo())
                with st.form("seg_cambiar_rol"):
                    ue = st.selectbox("Usuario existente", emails_gestion, key="seg_rol_user")
                    ...
                    if st.form_submit_button("ACTUALIZAR PERFIL") and ue:
                        if nuevo_rol == "super_admin" and not es_super_admin():
                            st.error("Solo el super administrador puede asignar ese perfil.")
                        elif not usuario_gestionable_demo(conn, ue):
                            st.error("No tiene permiso para modificar este usuario.")
                        else:
                ...
                with st.form("seg_cambiar_clave", clear_on_submit=True):
                    ...
                    ue_pw = st.selectbox("Usuario", emails_gestion, key="seg_pw_user")
                ...
                with st.form("seg_reenviar_invitacion", clear_on_submit=True):
                    ...
                    st.caption(
                        "Útil si el invitado no recibió el mail inicial (revise spam). "
                        "Debe indicar la contraseña que irá en el correo."
                    )
                    ue_re = st.selectbox("Usuario", emails_gestion, key="seg_reinv_user")
                    clave_re = st.text_input("Contraseña para el correo", type="password", key="seg_reinv_pw")
                    actualizar_pw = st.checkbox(
                        "Actualizar también la contraseña en el sistema",
                        value=True,
                        key="seg_reinv_upd",
                    )
                    if st.form_submit_button("REENVIAR INVITACIÓN") and ue_re:
                        if not clave_re.strip():
                            st.error("Ingrese la contraseña que se enviará en el correo.")
                        elif len(clave_re.strip()) < 4:
                            st.error("La contraseña debe tener al menos 4 caracteres.")
                        else:
                            row_re = conn.execute(
                                "SELECT rol, fecha_expira, invitado_por FROM usuarios WHERE email=?",
                                (ue_re,),
                            ).fetchone()
                            if not row_re:
                                st.error("Usuario no encontrado.")
                            else:
                                rol_re, fecha_re, inv_por = row_re
                                if actualizar_pw:
                                    conn.execute(
                                        "UPDATE usuarios SET password=? WHERE email=?",
                                        (hash_password(clave_re.strip()), ue_re),
                                    )
                                    conn.commit()
                                admin_re = st.session_state.get("email", "") or inv_por
                                mail_res = enviar_correo_invitacion_demo(
                                    ue_re, clave_re.strip(), rol_re, admin_re, fecha_re,
                                )
                                ok_inv = mail_res.get("invitado") if isinstance(mail_res, dict) else bool(mail_res)
                                ok_copia = mail_res.get("copia", False) if isinstance(mail_res, dict) else False
                                registrar_accion("INVITACION REENVIADA", ue_re)
                                respaldos = _destinatarios_admin_demo(admin_re)
                                if ok_inv:
                                    msg_re = f"Invitación reenviada a **{ue_re}**"
                                    if ok_copia and respaldos:
                                        msg_re += f" (copia a {', '.join(respaldos)})."
                                    st.success(msg_re)
                                elif ok_copia and respaldos:
                                    st.warning(
                                        f"No se pudo enviar a **{ue_re}**, pero sí la copia al admin. "
                                        "Revise bitácora → MAIL INVITACION."
                                    )
                                else:
                                    st.error(
                                        f"No se pudo reenviar a **{ue_re}**. Revise bitácora → FALLO_SMTP."
                                    )
                                st.rerun()
                st.divider()
                st.markdown("##### Eliminar usuario")
                st.warning("Esta acción es permanente. No puede eliminar su propio usuario ni el último administrador de campo.")
                with st.form("seg_eliminar_usuario"):
                    ue_del = st.selectbox("Usuario a eliminar", emails_gestion, key="seg_del_user")
                    ...
                    if st.form_submit_button("ELIMINAR USUARIO"):
                        ...
                        if not ue_del:
                            ...
                            st.error("Debe marcar la casilla de confirmación.")
                        elif not usuario_gestionable_demo(conn, ue_del):
                            st.error("No tiene permiso para eliminar este usuario.")
                        else:
                            rol_row = conn.execute("SELECT rol FROM usuarios WHERE email=?", (ue_del,)).fetchone()
                            rol_del = normalizar_rol_usuario(rol_row[0], ue_del) if rol_row else ""
                            n_super, n_cliente = contar_roles_admin_demo(conn)
                            if rol_del == "super_admin" and n_super <= 1:
                                st.error("No puede eliminar el único super administrador.")
                            elif rol_del == "admin_cliente" and n_cliente <= 1:
                                st.error("No puede eliminar el único administrador de campo.")
                            else:
                                ...
                                st.rerun()
            elif emails and not emails_gestion:
                st.info("Los demás usuarios son de plataforma (super administrador) y solo ellos pueden gestionarlos.")
    elif sec == "modulos":
            st.markdown("#### Módulos visibles por operador")
            st.caption(
                "Solo aplica a usuarios con perfil **operador**. Si no asigna módulos (o deja todos marcados), "
                "el operador ve el menú completo. El módulo **Manual** siempre queda disponible."
            )
            ops = pd.read_sql_query(
                """SELECT email, COALESCE(modulos, '') AS modulos
                   ...
                   AND (
                       ? = 1
                       OR lower(COALESCE(invitado_por, '')) = lower(?)
                   )
                   ORDER BY email""",
                conn,
                params=(1 if es_super_admin() else 0, st.session_state.get("email", "")),
            )
            if ops.empty:
                if es_super_admin():
                    st.info("No hay usuarios con perfil operador. Cree uno en la pestaña Usuarios y perfiles.")
                else:
                    st.info(
                        "No hay operadores asociados a su sesión en esta demo compartida. "
                        "En el ERP del cliente real podrá crear su equipo desde **Usuarios y perfiles**."
                    )
            else:
                ...
                with st.form("seg_modulos_operador"):
                    ...
                    for i, (lbl, key) in enumerate(MENU_COMPLETO):
                        with cols_mod[i % 3]:
                            if st.checkbox(lbl, key=f"seg_chk_mod_{mod_user_slug}_{key}"):
        ...
        _render_admin_plataforma_demo(conn)
    conn.close()
...
def login_page():
    ...
    hero = """
        <div class="login-hero-icon" aria-hidden="true">🚜</div>
        <h1>ERP DEMO AGRICOLA</h1>
        <p>Gestión integral para operaciones agrícolas</p>
        <span class="demo-badge">ENTORNO DEMO</span>
    """
    st.markdown(_login_fondo_html(hero), unsafe_allow_html=True)
    inyectar_sello_erpmaster_demo()
        ...
        if st.session_state.login_panel_abierto:
            ...
            with st.form("login"):
                ...
                if st.form_submit_button("ACCEDER"):
                    email_login = str(st.session_state.get("login_usuario", e) or "").strip()
                    clave_login = str(st.session_state.get("login_clave", p) or "")
                    conn = conectar_db()
                    try:
                        ...
                        if row_login:
                            ...
                            if not usuario_prueba_vigente(row_login[2]):
                                f_venc = pd.to_datetime(row_login[2]).strftime("%d-%m-%Y")
                                st.error(f"Su periodo de prueba finalizó el {f_venc}. Contacte al administrador del demo.")
                            else:
                                ...
                                st.session_state['rol'] = normalizar_rol_usuario(row_login[1], row_login[0])
                                st.session_state.login_panel_abierto = False
                                st.rerun()
                        else:
...
def run_streamlit_demo():
    ...
    if not st.session_state['logged_in']:
        login_page()
        return
    _conn_exp = conectar_db()
    _exp_row = _conn_exp.execute(
        "SELECT fecha_expira FROM usuarios WHERE email=?",
        (st.session_state.get("email", ""),),
    ).fetchone()
    _conn_exp.close()
    if _exp_row and _exp_row[0] and not usuario_prueba_vigente(_exp_row[0]):
        st.warning("Su periodo de prueba ha finalizado. La sesión se cerrará.")
    ...
    if 'rol' not in st.session_state:
        _conn_rol = conectar_db()
        _row_rol = _conn_rol.execute(
            "SELECT COALESCE(rol,'operador') FROM usuarios WHERE email=?",
            (st.session_state.get('email', ''),),
        ).fetchone()
        st.session_state['rol'] = normalizar_rol_usuario(
            _row_rol[0] if _row_rol else 'operador',
            st.session_state.get('email', ''),
        )
        _conn_rol.close()
    inyectar_css()
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="brand-sidebar">
                <p class="brand-title">🚜 ERP DEMO AGRICOLA</p>
                <p class="brand-sub">Versión demostración</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
        st.markdown(f"👤 <span class='sidebar-user'>{st.session_state['email']}</span>", unsafe_allow_html=True)
        _conn_vig = conectar_db()
        _row_vig = _conn_vig.execute(
            "SELECT fecha_expira FROM usuarios WHERE email=?",
            (st.session_state.get("email", ""),),
        ).fetchone()
        _conn_vig.close()
        if _row_vig and _row_vig[0]:
            _dias_v = dias_restantes_prueba(_row_vig[0])
            if _dias_v is not None and _dias_v >= 0:
                _f_vig = pd.to_datetime(_row_vig[0]).strftime("%d-%m-%Y")
                _color_v = "#E65100" if _dias_v <= 3 else "#1B5E20"
                st.markdown(
                    f"<span class='status-pill' style='background:#FFF8E1;border-color:#FFE082;color:{_color_v};'>"
                    f"⏳ Prueba demo: {_dias_v} día(s) — vence {_f_vig}</span>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    "<span class='status-pill' style='background:#E3F2FD;border-color:#90CAF9;color:#1565C0;"
                    "font-size:0.72rem;line-height:1.35;display:block;margin-top:0.35rem;'>"
                    "ℹ️ Sesión individual · datos ficticios compartidos entre invitados</span>",
                    unsafe_allow_html=True,
                )
        st.divider()
        _email_sb = st.session_state.get("email", "")
        _conn_rol_sb = conectar_db()
        _row_rol_sb = _conn_rol_sb.execute(
            "SELECT COALESCE(rol, 'operador') FROM usuarios WHERE email=?",
            (_email_sb,),
        ).fetchone()
        _conn_rol_sb.close()
        if _row_rol_sb:
            st.session_state["rol"] = normalizar_rol_usuario(_row_rol_sb[0], _email_sb)
        rol = _rol_sesion()
        rol_txt = etiqueta_perfil_demo(rol, _email_sb)
        ...
        try:
            ...
            m_opts = aplicar_badge_menu_soporte(m_opts, _conn_menu, es_admin)
            m_opts = aplicar_badge_menu_maquinaria(m_opts, _conn_menu)
        except Exception:
            pass
        finally:
            _conn_menu.close()
        ...
        st.divider()
        if st.button("🚪 CERRAR SESIÓN", key="btn_logout"): st.session_state.clear(); st.rerun()
    ...
    elif menu == "Campob": modulo_campob()
    elif menu == "Libro de Campo": modulo_libro_campo()
    elif menu == "GlobalGAP": modulo_globalgap()
    elif menu == "Maquinaria": modulo_maquinaria()
    elif menu == "Costos": modulo_costos()
    elif menu == "Flujo financiero": modulo_flujo_financiero()
    elif menu == "Soporte": modulo_soporte()
    elif menu == "Administracion": modulo_seguridad()
    elif menu == "Manual": modulo_manual()
