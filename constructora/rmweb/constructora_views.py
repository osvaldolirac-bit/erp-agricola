...
def register_constructora_routes(app, login_required):
    def _log(db, accion: str, detalle: str = "") -> None:
        usuario = (session.get("email") or session.get("usuario") or "SISTEMA").strip()
        bit.registrar_bitacora(db, usuario, accion, detalle)
        try:
            db.commit()
        except Exception:
            pass
