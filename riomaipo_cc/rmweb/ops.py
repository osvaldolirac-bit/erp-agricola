...
def ensure_ops_schema(c) -> None:
    ...
    try:
        ...
        _cst.ensure_constructora_schema(c)
    except Exception:
        pass
...
def registrar_movimiento_stock(
    ...
    *,
    ...
    cc_id = int(centro_costo_id) if centro_costo_id else None
    # Salidas manuales imputan a CC/obra (módulo Constructora).
    if tipo == "salida" and (origen or "").strip().lower() == "manual" and not cc_id:
        return False, "La salida de bodega requiere centro de costo / obra."
    if cc_id:
        cc = c.execute(
            "SELECT id FROM centros_costo WHERE id=? AND COALESCE(activo,1)=1",
            (cc_id,),
        ).fetchone()
        if not cc:
            return False, "Centro de costo / obra no válido."
    ...
    c.execute(
        ...
        (inventario_id, tipo, cantidad, costo_unitario, fecha, origen, origen_id, nota, centro_costo_id)
        VALUES (?,?,?,?,?,?,?,?,?)
        """,
        (
            inv_id,
